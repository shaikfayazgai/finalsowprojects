...
def list_workforce(
    ...
    # Also surface the REAL contributors who delivered this tenant's work
    # (their tasks → decomposition plan → tenant), so the directory reflects who
    # actually worked — not just CSV-imported internal staff. Deduped by email.
    seen = {(i.get("email") or "").lower() for i in items}
    needle = (search or "").strip().lower()
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT DISTINCT la.id, la.email, la.first_name, la.last_name,
                       cp.primary_skills, cp.secondary_skills, cp.availability
                  FROM contributor_tasks ct
                  JOIN decomp_plans dp ON dp.id = (ct.data->>'planId')
                  JOIN login_accounts la ON la.id = ct.account_id
            ...
            for r in cur.fetchall():
                ...
                name = (f"{r.get('first_name') or ''} {r.get('last_name') or ''}".strip()
                        or r.get("email"))
                if needle and needle not in email and needle not in name.lower():
                    continue
                seen.add(email)
                skills = (r.get("primary_skills") or []) + (r.get("secondary_skills") or [])
                items.append({
                    "userId": str(r["id"]),
                    "email": r["email"],
                    "displayName": name,
                    "department": "Contributor network",
                    "contribType": "freelancer",
        ...
        logger.warning("workforce contributor merge failed: %s", exc)
...
def assign_task(
    ...
    # Contributor pay for this task (major units, e.g. rupees). This becomes the
    # eligible payout once the work clears mentor + reviewer review. Manual for now
    # (enterprise sets it); AI/pricing-engine can populate it later.
    try:
        reward: float = float(body.get("reward") or body.get("amount") or 0)
    ...
    # Resolve the SOW this task belongs to (via the decomposition plan, or the
    # decomp task's plan) so it can flow with the work for delivery-review context.
    sow_id: str | None = (body.get("sowId") or "").strip() or None
    if not sow_id:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            if plan_id:
                cur.execute("SELECT sow_id FROM decomp_plans WHERE id=%s LIMIT 1", [plan_id])
            else:
                cur.execute(
                    "SELECT p.sow_id FROM decomp_tasks t JOIN decomp_plans p ON p.id=t.plan_id "
    ...
    if assignee_id:
        try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute(
                    "SELECT title, description, attachments, contributor_amount_minor, pay_currency, "
                    "estimated_hours, required_skills "
                    "FROM decomp_tasks WHERE id=%s LIMIT 1",
                    [task_id],
                )
                task_row = cur.fetchone()
            title = (task_row or {}).get("title") or f"Task {task_id}"
            task_desc = (task_row or {}).get("description")
            task_attachments = (task_row or {}).get("attachments") or []
            # Carry the task's required skills onto the contributor task so skill
            # reinforcement (digital-twin top skills) can be computed from real work.
            task_skills = (body.get("skills")
                           or (task_row or {}).get("required_skills") or [])
            # The contributor's pay comes from the SUPER-ADMIN price set at approval
            # (gross). It overrides any reward passed in the request.
            ca_minor = (task_row or {}).get("contributor_amount_minor")
            if ca_minor is not None:
                reward = float(ca_minor) / 100.0
                currency = (task_row or {}).get("pay_currency") or currency
            # Delivery deadline so on-time tracking works — explicit override from
            # the assigner, else derived from the task's estimated effort.
            from shared.deadlines import compute_due_at
            est_hours = (task_row or {}).get("estimated_hours")
            due_at = compute_due_at(
                estimated_hours=est_hours,
                override=body.get("dueAt") or body.get("deadline"),
            )
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """INSERT INTO contributor_tasks
                       ...
                       ON CONFLICT DO NOTHING""",
                    [_resolve_int, title, reward, currency, due_at,
                     _Json({"assignmentId": assignment_id, "taskId": task_id,
                            "planId": plan_id, "sowId": sow_id, "reason": reason,
                            "reward": reward, "currency": currency,
                            "dueAt": due_at.isoformat(),
                            "estimatedHours": float(est_hours) if est_hours else None,
                            "skills_required": task_skills,
                            "description": task_desc, "attachments": task_attachments})],
                )
            conn.commit()
        except Exception as exc:  # noqa: BLE001
...
def reassign_task(
    ...
    if new_assignee_id:
        try:
            ...
            with conn.cursor() as cur:
                cur.execute("SELECT title, estimated_hours FROM decomp_tasks WHERE id=%s LIMIT 1", [task_id])
                task_row = cur.fetchone()
                title = task_row[0] if task_row else f"Task {task_id}"
                est_hours = task_row[1] if (task_row and len(task_row) > 1) else None
            # Fresh delivery deadline for the new assignee (override or derived).
            from shared.deadlines import compute_due_at
            due_at = compute_due_at(
                estimated_hours=est_hours,
                override=body.get("dueAt") or body.get("deadline"),
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """INSERT INTO contributor_tasks
                       (account_id, title, status, priority, category, due_at, data)
                       VALUES (%s,%s,'assigned','normal','delivery',%s,%s)
                       ON CONFLICT DO NOTHING""",
                    [_resolve_int, title, due_at,
                     _Json({"assignmentId": assignment_id, "taskId": task_id,
