...
function normalizeManualList(res: unknown): NormalisedSOW[] {
  ...
  return arr.map((item) => {
    ...
    return {
      ...
      title:      String(item.projectTitle ?? item.title ?? item.project_title ?? item.document_title ?? "Untitled").replace(/^statement of work for\s*/i, "").trim(),
      client:     String(item.clientOrganisation ?? item.client ?? item.client_organisation ?? ""),
