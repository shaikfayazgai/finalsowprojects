...
import { listInvoices, type ApiInvoice } from "@/lib/api/enterprise-billing";
...
export default function BillingDashboardPage() {
  ...
  const billingToken = (billingSession?.user as { accessToken?: string } | undefined)?.accessToken ?? "";
  const [invoices, setInvoices] = React.useState<ApiInvoice[]>(mockInvoices as unknown as ApiInvoice[]);

  React.useEffect(() => {
    if (!billingToken) return;
    let cancelled = false;
    listInvoices(billingToken)
      .then((d) => { if (!cancelled && d?.invoices?.length) setInvoices(d.invoices); })
      .catch(() => { /* keep fallback */ });
