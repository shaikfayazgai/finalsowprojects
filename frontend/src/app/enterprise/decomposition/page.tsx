...
export default function DecompositionPlansPage() {
  ...
  const allPlans: DecompositionPlan[] = React.useMemo(() => {
    ...
    return rawArr.map((p) => ({
      ...
      title: String(p.projectTitle ?? p.title ?? p.project_name ?? "Untitled Plan").replace(/^statement of work for\s*/i, "").trim(),
  ...
  const handleKickoff = (plan: DecompositionPlan) => {
    // Already kicked off this session → just open the plan to manage tasks.
    if (justPaidIds.has(plan.id)) {
      router.push(`/enterprise/decomposition/${plan.id}`);
      return;
    }
    // Otherwise show the (simulated) payment, which then kicks off + opens the plan.
    setPaymentPlan(plan);
  };

  const handleViewPlan = (plan: DecompositionPlan) => {
    // Open the plan in manual mode (no AI-generation query param).
    router.push(`/enterprise/decomposition/${plan.id}`);
  };
  ...
  const handlePaymentSuccess = (paidPlanId: string) => {
    ...
    setPaymentPlan(null);
    // After the (simulated) payment, kick off + confirm the plan, then open it in
    // MANUAL mode so the enterprise can add tasks. No AI generation step.
    kickoffActionMutation.mutate(paidPlanId, {
      onSettled: () => {
        approveKickoffMutation.mutate(paidPlanId, {
          onSettled: () => {
            kickoffActionMutation.reset();
            approveKickoffMutation.reset();
            router.push(`/enterprise/decomposition/${paidPlanId}`);
          },
        });
      },
    });
        ...
        >
          ...
          <div className="px-7 pt-6 pb-7">
            ...
            <p className="text-[13px] text-gray-600 leading-relaxed mb-3">
              You are about to start task decomposition for:
            </p>
            ...
            <p className="text-[12.5px] text-gray-500 leading-relaxed mb-1">
              Once confirmed, the plan is locked in and you can break the SOW into tasks and milestones, then assign contributors. <span className="font-semibold text-gray-700">This process cannot be interrupted once started.</span>
            </p>
    ...
    <motion.div variants={stagger} initial="hidden" animate="show">
      ...
      <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-7">
        <div>
          ...
          <p className="text-[13px] text-gray-500 mt-1.5">Break approved SOWs into tasks and milestones, then assign contributors.</p>
        </div>
      ...
      <motion.div variants={fadeUp} className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-7">
        {[
          ...
          { label: "Confirmed Plans", value: `${avgConfidence}%`, icon: BrainCircuit, iconBg: "bg-gradient-to-br from-gold-400 to-gold-600" },
