...
import { fetchMentorQueue } from "@/lib/api/mentor";
import {
...
/** Map a backend review row (mentor-service) → this page's QueueItem shape. */
function mapQueueItem(r: Record<string, unknown>): QueueItem {
  const get = <T,>(k: string, d: T): T => (r[k] === undefined || r[k] === null ? d : (r[k] as T));
  return {
    id: String(get("id", "")),
    taskTitle: get("title", "Untitled"),
    contributorId: get("contributor_name", "—"),
    projectTitle: get("project_title", get("submission_type", "—")),
    submittedAt: get("created_at", ""),
    slaDeadline: get("sla_deadline", get("updated_at", "")),
    status: get("status", "pending"),
    urgency: get("priority", "normal"),
    reviewType: get("review_type", "initial"),
    submissionVersion: get("submission_version", 1),
    qualityScore: get<number | null>("score", null),
    skillsInvolved: get<string[]>("skills", []),
  };
}
...
export default function ReviewQueuePage() {
  ...
  React.useEffect(() => {
    ...
    fetchMentorQueue(token)
      .then((d) => { if (!cancelled) setQueue((d?.items ?? []).map((r) => mapQueueItem(r as unknown as Record<string, unknown>))); })
      .catch(() => { if (!cancelled) setQueue([]); })
      .finally(() => { if (!cancelled) setLoadingQueue(false); });
  ...
  const inReviewCount = queue.filter((r) => r.status === "in_review").length;
  const completedCount = queue.filter((r) => r.status === "completed").length;
  const escalatedCount = queue.filter((r) => r.status === "escalated").length;
  ...
  const filteredItems = React.useMemo(() => {
    ...
    return list;
  }, [queue, statusFilter, urgencyFilter, typeFilter, searchQuery, sortField, sortDir]);
