...
import { fetchReview, decideReview, type QueueItem } from "@/lib/api/mentor";
import {
...
export default function ReviewDetailPage() {
  ...
  // Fetch the real review; fall back to mock display fields for the rich UI
  // sections that don't have live data yet (rubric, checklist demo content).
  const [live, setLive] = React.useState<(QueueItem & { payload?: Record<string, unknown> }) | null>(null);
  React.useEffect(() => {
    if (!token || !reviewId) return;
    let cancelled = false;
    fetchReview(token, reviewId)
      .then((r) => { if (!cancelled) setLive(r as QueueItem & { payload?: Record<string, unknown> }); })
      .catch(() => {});
  ...
  const item = {
    ...mockReviewItem,
    taskTitle: live?.title ?? mockReviewItem.taskTitle,
    contributorId: (live?.contributor_name as string) ?? mockReviewItem.contributorId,
    status: live?.status ?? "pending",
  };
  const submissionUrl = (live?.payload as { url?: string } | undefined)?.url;
  ...
  async function confirmDecision() {
    ...
    // Map UI decision → backend decision. The mentor backend accepts
    // accept | rework | escalate; a hard reject escalates to the platform.
    const backendDecision = decision === "reject" ? "escalate" : (decision ?? "accept");
    const comments = decision === "rework" ? reworkDetails
      : decision === "reject" ? rejectReason
      : feedback;
    const score = allScored ? Number(avgScore) : undefined;
    setSubmitting(true);
    try {
      await decideReview(token, reviewId, { decision: backendDecision, score, feedback: comments });
      const labels: Record<string, string> = { accept: "Accepted", rework: "Rework Requested", reject: "Rejected" };
      toast.success(`Review submitted — ${labels[decision!]}`,
        decision === "accept" ? "Routed to the enterprise reviewer." : `${item.contributorId} has been notified`);
      setShowDecisionConfirm(false);
      router.push("/mentor/queue");
    } catch {
      toast.error("Could not submit the review. Try again.");
    } finally {
      setSubmitting(false);
    }
  }
