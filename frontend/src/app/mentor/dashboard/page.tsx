...
import { fetchMentorDashboard, type MentorStats } from "@/lib/api/mentor";
...
export default function MentorDashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchMentorDashboard(token)
      ...
      .catch(() => { /* keep nulls → fall back to dashes */ });
    return () => { cancelled = true; };
  }, [token]);

  if (status === "loading") return <MentorDashboardSkeleton />;

  const queueSize = stats ? String(stats.pending_reviews) : "—";
  const completed = stats ? String(stats.completed_reviews) : "—";
  const totalReviews = stats?.total_reviews ?? 0;
  const approvalRate = stats && totalReviews > 0
    ? `${Math.round((stats.completed_reviews / totalReviews) * 100)}%` : "—";
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          label="Queue Size"
          value={queueSize}
          change={stats ? `${stats.escalations} escalated` : ""}
          changeType="neutral"
        ...
        <StatCard
          ...
          label="Approval Rate"
          value={approvalRate}
          change={stats ? `${totalReviews} total` : ""}
          changeType="neutral"
