      ...
      >
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          <div className="px-6 py-6 space-y-4 pb-4">
            ...
            {/* ── Assign QA mentor (platform admin, at the Commercial gate) ── */}
            <AssignMentorPanel sowId={sowId} />
...
/* ── Assign QA mentor to this SOW (Super Admin / Commercial gate) ────────────
   The chosen mentor receives this project's contributor deliverables in their
   review queue. If none is assigned, submissions fall to the open pool. */
function AssignMentorPanel({ sowId }: { sowId: string }) {
  type Mentor = { id: string; email: string; name: string };
  const [mentors, setMentors] = React.useState<Mentor[]>([]);
  const [assigned, setAssigned] = React.useState<{ mentorId?: string; mentorName?: string; mentorEmail?: string } | null>(null);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const m = await fetch("/api/superadmin/mentors", { cache: "no-store" }).then((r) => r.json());
        if (!off) setMentors(m.mentors ?? []);
      } catch { /* ignore */ }
      try {
        const a = await fetch(`/api/superadmin/sows/${sowId}/mentor`, { cache: "no-store" }).then((r) => r.json());
        if (!off && a?.mentor?.mentorId) { setAssigned(a.mentor); setPicked(a.mentor.mentorId); }
      } catch { /* ignore */ }
    })();
    return () => { off = true; };
  }, [sowId]);
  ...
  async function save() {
    const m = mentors.find((x) => x.id === picked);
    if (!m) { setMsg("Pick a mentor first."); return; }
    setBusy(true); setMsg(null);
    try {
      const r = await fetch(`/api/superadmin/sows/${sowId}/assign-mentor`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mentorId: m.id, mentorEmail: m.email, mentorName: m.name }),
      });
      if (!r.ok) throw new Error();
      setAssigned({ mentorId: m.id, mentorName: m.name, mentorEmail: m.email });
      setMsg(`Assigned ${m.name} — their queue will receive this project's submissions.`);
    } catch {
      setMsg("Could not assign mentor. Try again.");
  ...
  return (
    <div className="rounded-xl bg-white border border-gray-100 shadow-sm px-5 py-4">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="text-[13px] font-semibold text-brown-950">QA Mentor assignment</h3>
      </div>
      <p className="text-[11.5px] text-beige-500 mb-3">
        Assign the mentor who reviews this project&apos;s contributor deliverables. Unassigned → open pool.
      </p>
      {assigned?.mentorId && (
        <p className="mb-2 text-[12px] text-forest-700">
          Currently assigned: <span className="font-semibold">{assigned.mentorName || assigned.mentorEmail}</span>
        </p>
      )}
      <div className="flex flex-wrap items-center gap-2">
        <select value={picked} onChange={(e) => setPicked(e.target.value)} disabled={busy}
          className="h-9 min-w-[220px] rounded-lg border border-gray-200 bg-white px-3 text-[12.5px] text-gray-700 outline-none focus:border-brown-300">
        ...
        <button onClick={save} disabled={busy || !picked}
          ...
          {busy ? "Saving…" : assigned?.mentorId ? "Reassign" : "Assign mentor"}
        </button>
      </div>
      {msg && <p className="mt-2 text-[11.5px] text-beige-600">{msg}</p>}
    </div>
  );
}
