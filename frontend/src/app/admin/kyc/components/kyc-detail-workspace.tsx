...
import { useAdminKycCase, useKycDecision, useKycPhotoReveal } from "@/lib/hooks/use-admin-kyc";
...
export function KycDetailWorkspace() {
  ...
  const { submit: submitDecisionLive, submitting } = useKycDecision();
  ...
  function handleSubmit() {
    if (!canSubmit || !decision || submitting) return;

    // Backend supports approve | reject only; "more_info" maps to reject + note.
    const liveDecision = decision === "approved" ? "approve" : "reject";

    void submitDecisionLive(caseId, liveDecision, note.trim() || undefined, () => {
      // Keep the local mock overlay in sync so the queue reflects the outcome
      // immediately even if a subsequent live refresh is delayed.
      const updated = submitKycDecision(caseId, decision, note, profile.displayName);

      if (
        decision === "approved" &&
        updated &&
        (updated.track === "Freelancer" || updated.track === "Women WF")
      ) {
        void markContributorKycVerifiedAction(updated.contributorEmail).then((result) => {
          if (!result.success) {
            console.error("[kyc approve]", result.error);
          }
        });
      }
