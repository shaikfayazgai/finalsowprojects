...
export function NewMentorWorkspace() {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    void (async () => {
      ...
      setError(null);
      try {
        // Unified provisioning flow: create the mentor login account and email
        // credentials (email + temp password) — same path as tenants/students/
        // women. The mentor's role tier maps to the highest selected role.
        const [first, ...restName] = name.trim().split(" ");
        const mentorRole = roles.lead ? "mentor.lead" : roles.senior ? "mentor.senior" : "mentor";
        ...
        router.push(`/admin/mentors/${created.id}?invited=1`);
      } catch (err) {
        setError(
