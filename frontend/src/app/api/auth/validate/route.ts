...
export async function POST(req: NextRequest) {
  try {
    ...
    // Provisioned account whose password isn't set yet (enterprise/mentor/admin/
    // student get emailed credentials). Backend emails an OTP and returns this
    // status; the login form routes the user to the setup-password page.
    if (status === "needs_password_setup") {
      return NextResponse.json({
        ok: true,
        status: "needs_password_setup",
        user: { id: userObj.id ?? "", email: userObj.email ?? email },
      });
    }
