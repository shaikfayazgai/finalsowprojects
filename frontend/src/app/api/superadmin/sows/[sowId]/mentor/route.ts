...
export async function GET(req: NextRequest, { params }: { params: Promise<{ sowId: string }> }) {
  ...
  try {
    const r = await fetch(`${BACKEND}/api/superadmin/sows/${sowId}/mentor`, { headers, cache: "no-store", signal: AbortSignal.timeout(30000) });
    return NextResponse.json(await r.json().catch(() => ({})), { status: r.status });
  } catch {
    return NextResponse.json({ mentor: null }, { status: 200 });
  }
}
