...
// List mentor accounts available for SOW assignment.
export async function GET(req: NextRequest) {
  ...
  try {
    const r = await fetch(`${BACKEND}/api/superadmin/mentors`, { headers, cache: "no-store", signal: AbortSignal.timeout(30000) });
    return NextResponse.json(await r.json().catch(() => ({})), { status: r.status });
  } catch {
    return NextResponse.json({ mentors: [], total: 0 }, { status: 200 });
  }
}
