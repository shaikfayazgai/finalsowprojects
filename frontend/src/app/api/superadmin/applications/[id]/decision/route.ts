...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }){const { id } = await params;try{const b=await req.text();const r=await fetch(`${BACKEND}/api/superadmin/applications/${id}/decision`,{method:"POST",headers:H(req),body:b,signal:AbortSignal.timeout(30000)});return NextResponse.json(await r.json().catch(()=>({})),{status:r.status});}catch{return NextResponse.json({detail:"Failed"},{status:500});}}
