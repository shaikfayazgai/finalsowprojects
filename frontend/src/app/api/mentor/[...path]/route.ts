...
/**
 * Catch-all proxy for the mentor portal → mentor-service (via Kong).
 * Forwards GET/POST/PATCH/DELETE for /api/mentor/* with the Bearer token.
 */
...
async function forward(req: NextRequest, path: string[], method: string) {
  ...
  const url = `${BACKEND}/api/mentor/${sub}${qs}`;
