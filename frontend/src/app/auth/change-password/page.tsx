...
function ChangePasswordContent() {
  ...
  useEffect(() => {
    ...
    let em = "";
    try {
      tok = sessionStorage.getItem("_first_login_token") ?? "";
      em  = sessionStorage.getItem("_first_login_email") ?? "";
    } catch {
      /* sessionStorage unavailable */
    }
    // Forced-reset-after-login flow: the user is already signed in with the
    // temp password, so fall back to the live session token.
    if (!tok && sessionToken) {
      tok = sessionToken;
      em = session?.user?.email ?? "";
    }
    if (tok) { setAccessToken(tok); setEmail(em); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewPassword("");
      setConfirmPassword("");
      // Password is changed and the backend cleared must_change_password. The
      // current session still carries the stale requiresPasswordChange flag, so
      // sign out and bounce to home — the user signs in fresh with their new
      // password and is then routed to onboarding/dashboard normally.
      setTimeout(() => { void signOut({ callbackUrl: "/" }); }, 2000);
    } catch (err: unknown) {
