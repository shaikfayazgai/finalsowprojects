...
export function buildApprovedMessage(sowId: string, stageIndex: number, sowTitle: string, submitterName: string): MsgBase {
  // Clamp to the valid stage range — there are only 5 stages (idx 0-4). Past
  // the last stage there's no "next" stage and STAGE_META is undefined.
  const safeIdx = Math.min(Math.max(stageIndex, 0), STAGE_META.length - 1);
  const stage = STAGE_META[safeIdx];
  const nextStage = STAGE_META[safeIdx + 1];
  return {
    sowId,
    stageIndex: safeIdx,
    stageKey: stage.key,
