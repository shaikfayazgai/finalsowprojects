...
export function MilestonePaymentModal({
  ...
  const handlePay = async (milestone: MilestonePayment) => {
    ...
    // SIMULATED payment (no real Razorpay) — show a brief processing state then
    // succeed, so the kickoff flow proceeds without a live payment gateway.
    setPaymentStatus("processing");
    setTimeout(() => triggerSuccess(milestone), 900);
  };
