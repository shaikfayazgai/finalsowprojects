...
export const decompositionApi = {
  ...
  /** POST .../plans/{plan_id}/tasks — Add a task manually */
  createTask(planId: string, body: { title: string; detail?: { description?: string }; skills?: string[] }): Promise<BaseResponse> {
    return decompositionCall<BaseResponse>(`/api/v1/enterprise/decomposition/plans/${planId}/tasks`, "POST", body);
  },

  /** GET .../contributors — assignable contributors for the dropdown */
  listContributors(): Promise<BaseResponse> {
    return decompositionCall<BaseResponse>(`/api/v1/enterprise/decomposition/contributors`);
  },

  /** POST .../plans/{plan_id}/tasks/{task_id}/assign — assign a task to a contributor */
  assignTask(planId: string, taskId: string, body: { contributorId: string; contributorEmail?: string; skills?: string[] }): Promise<BaseResponse> {
    return decompositionCall<BaseResponse>(`/api/v1/enterprise/decomposition/plans/${planId}/tasks/${taskId}/assign`, "POST", body);
  },
