...
// Upload a SOW through the SAME-ORIGIN Next proxy (/api/sow/upload). The proxy
// resolves the logged-in user's own token from the session and forwards the
// multipart file to the backend — so the SOW is created under the correct owner
// and the request reliably reaches the server (a direct cross-origin browser
// POST to BASE_URL was silently failing, so SOWs never persisted).
...
async function uploadViaProxy(
  ...
  fd.append("file", file);
  fd.append("projectTitle", metadata.projectTitle);
  fd.append("clientOrganisation", metadata.clientOrganisation);
  if (metadata.linkedSowId) fd.append("linkedSowId", metadata.linkedSowId);
  if (metadata.budgetAmount) fd.append("budgetAmount", metadata.budgetAmount);
  if (metadata.budgetCurrency) fd.append("budgetCurrency", metadata.budgetCurrency);
  if (metadata.startDate) fd.append("startDate", metadata.startDate);
  if (metadata.endDate) fd.append("endDate", metadata.endDate);
  const res = await fetch(`/api/sow/upload${approved ? "?approved=1" : ""}`, {
    method: "POST",
    body: fd,
  });
...
export const sowApi = {
  ...
  uploadSOW(file: File, metadata: UploadMeta): Promise<BaseResponse> {
    return uploadViaProxy(file, metadata, false);
  },

  /** Upload via the same-origin proxy (carries the logged-in user's identity,
   *  so the SOW is created under the RIGHT owner — was failing as a direct
   *  cross-origin call before). */
  uploadSOWApproved(file: File, metadata: UploadMeta): Promise<BaseResponse> {
    return uploadViaProxy(file, metadata, true);
  },
  ...
  /** Close out a SOW after all its tasks are accepted (payout + credentials). */
  closeSOW(sowId: string, note?: string): Promise<BaseResponse> {
    return sowCall<BaseResponse>(`/api/v1/sow/${sowId}/close`, "POST", { note });
  },
