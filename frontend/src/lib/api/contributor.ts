...
// ── Profile completion (skills + projects + experience + education + expertise) ──
...
export interface ProfileProject {
  id: number; title: string; description: string; role?: string | null;
  url?: string | null; skills?: string[]; start_date?: string | null; end_date?: string | null;
}
export interface ProfileExperience {
  id: number; kind: string; organization: string; role: string;
  description?: string | null; location?: string | null;
  start_date?: string | null; end_date?: string | null; is_current?: boolean;
}
export interface ProfileEducation {
  id: number; institution: string; degree?: string | null; field?: string | null;
  grade?: string | null; start_year?: string | null; end_year?: string | null;
}

export function fetchProfileCompletion(token: string) {
  return apiCall<ProfileCompletion>("/api/contributor/profile/completion", { method: "GET", token });
}
export function listProjects(token: string) {
  return apiCall<ProfileProject[]>("/api/contributor/profile/projects", { method: "GET", token });
}
export function addProject(token: string, body: Partial<ProfileProject>) {
  return apiCall<ProfileProject>("/api/contributor/profile/projects", { method: "POST", token, body: JSON.stringify(body) });
}
export function deleteProject(token: string, id: number) {
  return apiCall(`/api/contributor/profile/projects/${id}`, { method: "DELETE", token });
}
export function listExperience(token: string) {
  return apiCall<ProfileExperience[]>("/api/contributor/profile/experience", { method: "GET", token });
}
export function addExperience(token: string, body: Partial<ProfileExperience>) {
  return apiCall<ProfileExperience>("/api/contributor/profile/experience", { method: "POST", token, body: JSON.stringify(body) });
}
export function deleteExperience(token: string, id: number) {
  return apiCall(`/api/contributor/profile/experience/${id}`, { method: "DELETE", token });
}
export function listEducation(token: string) {
  return apiCall<ProfileEducation[]>("/api/contributor/profile/education", { method: "GET", token });
}
export function addEducation(token: string, body: Partial<ProfileEducation>) {
  return apiCall<ProfileEducation>("/api/contributor/profile/education", { method: "POST", token, body: JSON.stringify(body) });
}
export function deleteEducation(token: string, id: number) {
  return apiCall(`/api/contributor/profile/education/${id}`, { method: "DELETE", token });
}
export function updateExpertise(token: string, expertise_areas: string[]) {
  return apiCall<ProfileCompletion>("/api/contributor/profile/expertise", { method: "PATCH", token, body: JSON.stringify({ expertise_areas }) });
}
...
export async function fetchTasks(
  ...
  const qs = q.toString();
  const res = await apiCall<TasksListResponse>(
    `/api/contributor/tasks${qs ? `?${qs}` : ""}`,
    { method: "GET", token, cache: "no-store" },
  );
  // Normalise each task so the list UI never hits an undefined field. The live
  // backend returns a leaner shape (reward/currency/due_at) than the rich
  // TaskItem the UI was built against — fill the gaps with safe defaults.
  const rawItems = (res.items ?? []) as unknown as Array<Record<string, unknown>>;
  return { ...res, items: rawItems.map(normalizeTaskItem) };
}

function normalizeTaskItem(t: Record<string, unknown>): TaskItem {
  const num = (v: unknown, d = 0) => (typeof v === "number" ? v : Number(v) || d);
  const str = (v: unknown, d = "") => (typeof v === "string" ? v : d);
  const arr = (v: unknown) => (Array.isArray(v) ? (v as string[]) : []);
  const reward = num(t.reward);
  const currency = str(t.currency, "USD");
  const hours = num(t.estimated_hours);
  return {
    id: String(t.id ?? ""),
    title: str(t.title, "Untitled task"),
    project_title: str(t.project_title),
    project_id: t.project_id as string | undefined,
    milestone_title: str(t.milestone_title),
    status: str(t.status, "available"),
    priority: str(t.priority, "medium"),
    skills_required: arr(t.skills_required),
    estimated_hours: hours,
    pricing: (t.pricing as TaskItemPricing) ?? { amount: reward, currency, model: "fixed" },
    match_score: num(t.match_score),
    match_reason: str(t.match_reason),
    due_date: str(t.due_date) || str(t.due_at),
    sla_deadline: str(t.sla_deadline),
    progress_percent: num(t.progress_percent),
    hours_logged: num(t.hours_logged),
    domain_tag: str(t.domain_tag) || str(t.category),
    seniority_required: str(t.seniority_required),
    contributor_seniority: str(t.contributor_seniority),
    skills_matched: arr(t.skills_matched),
    offer_expires_at: (t.offer_expires_at as string | null) ?? null,
    offered_at: (t.offered_at as string | null) ?? null,
    data_sensitivity: str(t.data_sensitivity, "internal"),
    nda_required: Boolean(t.nda_required),
    effort_display: str(t.effort_display) || (hours ? `${hours}h` : ""),
  };
}
...
export async function fetchSubmissions(
  ...
  if (params.page_size !== undefined) q.set("page_size", String(params.page_size));
  const qs = q.toString();
  const res = await apiCall<SubmissionsListResponse>(
    `/api/contributor/submissions${qs ? `?${qs}` : ""}`,
    { method: "GET", token, cache: "no-store" },
  );
  // Normalise: the backend submission has created_at (not submitted_at), no
  // version, and no task_title. Map to what the Submissions UI renders.
  const raw = (res.items ?? []) as unknown as Array<Record<string, unknown>>;
  const items: SubmissionItem[] = raw.map((s) => ({
    id: String(s.id ?? ""),
    task_id: String(s.task_id ?? ""),
    task_title: (s.task_title as string) || (s.summary as string) || `Task ${s.task_id ?? ""}`,
    version: typeof s.version === "number" ? s.version : 1,
    submitted_at: (s.submitted_at as string) || (s.created_at as string) || "",
