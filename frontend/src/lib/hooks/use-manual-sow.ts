...
export function useSOWDetail(sowId: string | null): {
  ...
  // Decide the flow from the SOW's OWN source/intake_mode, not from which
  // endpoint responded — both AI and manual SOWs live in the same
  // enterprise_sows table, so the AI endpoint also returns manual rows. A
  // manual upload has source="manual" (or intake_mode="manual_upload"); only
  // treat it as AI when it explicitly says so.
  const src = data
    ? String(
        (data.source as string | undefined) ??
        (data.intake_mode as string | undefined) ??
        (data.intakeMode as string | undefined) ??
        "",
      ).toLowerCase()
    : "";
  const flow: "manual" | "ai" =
    src === "manual" || src === "manual_upload"
      ? "manual"
      : src === "ai" || src === "ai_generated"
        ? "ai"
        : // No explicit marker → fall back to which endpoint actually had it.
          !aiData && !!manualData
          ? "manual"
          : "ai";
