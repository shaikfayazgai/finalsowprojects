 ...
 * Loads real escalation data from the backend.
 * Adjudication submit is still a local no-op (no backend adjudication endpoint yet).
 */
...
export default function EscalationDetailPage() {
  ...
  React.useEffect(() => {
    ...
    fetchRealMentorEscalation(id, c.signal)
  ...
  const onSubmit = async () => {
    if (!valid) return;
    setSubmitting(true);
    // TODO: wire to PATCH /api/mentor/escalation/{id} when adjudication endpoint is added.
    ...
    <MentorPage>
      ...
      <DashboardSection title="Context">
        <div className="space-y-3">
          ...
          {esc.rejectReason && (
            <div>
              <p className={mentorFieldLabel}>Escalation description</p>
