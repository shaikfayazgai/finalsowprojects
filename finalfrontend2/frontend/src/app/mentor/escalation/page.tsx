 ...
 * Shows this mentor's escalations from the real backend.
 * The senior/lead role filter from the mock is preserved but the data is
 * now real (escalations created when a mentor escalates a review decision).
 */
...
export default function MentorEscalationPage() {
  ...
  React.useEffect(() => {
    ...
    fetchRealMentorEscalations(c.signal)
      .then((res) => {
        setItems(res.items);
      ...
      .catch((err: unknown) => {
        ...
        setError(
          err instanceof MentorHistoryApiError ? err.message : "Could not load escalations.",
        );
      });
    return () => c.abort();
  }, [isSeniorOrLead]);
