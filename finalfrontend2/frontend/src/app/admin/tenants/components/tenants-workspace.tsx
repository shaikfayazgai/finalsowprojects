...
import { resolveProvisioningSteps } from "@/lib/admin/tenant-registry";
...
export function TenantsWorkspace() {
  ...
  // Real-data fetch state
  const [apiTenants, setApiTenants] = React.useState<MockTenant[] | null>(null);
  const [fetchError, setFetchError] = React.useState<string | null>(null);

  React.useEffect(() => {
    let cancelled = false;
    fetchAdminTenants()
      .then((rows) => { if (!cancelled) setApiTenants(rows); })
      .catch((err: unknown) => {
        if (!cancelled) {
          console.error("[TenantsWorkspace] fetch failed:", err);
  ...
  /**
   * Merge strategy:
   *   1. Start with real API tenants (de-duplicated by id).
   *   2. Prepend locally-provisioned tenants from the store (those not yet
   *      persisted to the backend or created mid-session).
   *   3. Apply per-tenant status/tier overrides from the store (pause, tier change).
   */
  const allTenants = React.useMemo<MockTenant[]>(() => {
    const base: MockTenant[] = apiTenants ?? [];
    const apiIds = new Set(base.map((t) => t.id));

    // Apply overrides to API tenants
    const overridden = base.map((t) => {
      const ov = tenantOverrides[t.id];
      if (!ov) return t;
      return { ...t, ...(ov.status ? { status: ov.status } : {}), ...(ov.tier ? { tier: ov.tier } : {}) };
    });

    // Locally-provisioned tenants not yet in the API response
    const localOnly: MockTenant[] = dynamicTenants
    ...
    return [...localOnly, ...overridden];
  }, [apiTenants, dynamicTenants, tenantOverrides]);
  ...
  if (!hydrated || apiTenants === null) return <TenantsSkeleton />;

  if (fetchError && apiTenants.length === 0 && dynamicTenants.length === 0) {
    return (
      <div className="space-y-5 pb-4 animate-fade-in">
        <div className="rounded-lg border border-error-border bg-error-subtle px-4 py-3 font-body text-[13px] text-error-text">
          Could not load tenants: {fetchError}
        </div>
      </div>
    );
  }
