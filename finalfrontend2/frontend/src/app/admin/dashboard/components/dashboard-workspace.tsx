...
export function AdminDashboardWorkspace() {
  ...
  // While loading or on error, use a zero-count placeholder so all the
  // downstream helpers receive a valid MockAdminDashboard shape.
  const d = dashState.status === "ok"
    ? dashState.data
    : {
        env: "PROD" as const,
        greetingFor: "",
        kpis: { servicesUp: 0, servicesTotal: 0, tenants: 0, mentors: 0, activeSows: 0 },
        pipeline: { tenantsActive: 0, commercialGate: 0, governanceOpen: 0, kycPending: 0, mentorsActive: 0 },
  ...
  if (dashState.status === "loading") {
    return (
      <div className="space-y-6 pb-4 animate-fade-in">
        <div className="h-8 w-48 rounded-lg bg-bg-subtle animate-pulse" />
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          <div className="lg:col-span-2 h-64 rounded-xl bg-bg-subtle animate-pulse" />
          <div className="h-64 rounded-xl bg-bg-subtle animate-pulse" />
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6 pb-4 animate-fade-in">
        <p className="font-body text-[13.5px] text-text-secondary">
          Dashboard data could not be loaded: {dashState.message}
        </p>
      </div>
    );
  }
