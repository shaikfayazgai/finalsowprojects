...
import { useAdminKycCase, useAdminKycCasesState } from "@/lib/hooks/use-admin-kyc";
...
import { KycDetailSkeleton } from "./kyc-detail-skeleton";

/** Reconstruct the masked ID string from the last-4 digits stored in the case. */
function fullKycIdNumber(c: MockKycCase): string {
...
export function KycDetailWorkspace() {
  ...
  const kycState = useAdminKycCasesState();
  const c = useAdminKycCase(params.caseId);

  // Local reveal state — no backend endpoint; access is logged client-side only.
  const [photoReveal, setPhotoReveal] = React.useState<{ revealedAt: string; by: string } | undefined>(undefined);
  const [photoOpen, setPhotoOpen] = React.useState(false);
  const [decisionOpen, setDecisionOpen] = React.useState(false);

  if (kycState.status === "loading") return <KycDetailSkeleton />;
  ...
  function handleRevealPhoto() {
    // Record reveal locally (no backend endpoint for photo reveal).
    setPhotoReveal({ revealedAt: new Date().toISOString(), by: profile.displayName });
    setPhotoOpen(true);
  }
