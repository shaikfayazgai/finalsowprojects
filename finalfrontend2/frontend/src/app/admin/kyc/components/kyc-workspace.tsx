...
import { useAdminKycCasesState } from "@/lib/hooks/use-admin-kyc";
...
import { KycSkeleton } from "./kyc-skeleton";
...
export function KycWorkspace() {
  ...
  const allCases = kycState.status === "ok" ? kycState.cases : [];
  ...
  if (kycState.status === "loading") return <KycSkeleton />;
  ...
  if (kycState.status === "error") {
    return (
      <div className="space-y-5 pb-4 animate-fade-in">
        ...
        <div className="rounded-lg border border-error-border bg-error-subtle/60 px-4 py-3">
          <p className="font-body text-[13px] text-error-text">
            <span className="font-semibold">Failed to load KYC cases.</span>{" "}
            {kycState.message}
          </p>
        </div>
      </div>
