...
export function NewMentorModal({ open, onClose }: NewMentorModalProps) {
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    void (async () => {
      ...
      setError(null);
      try {
        // Create the mentor account via the real superadmin users endpoint.
        ...
        if (!res.ok) throw new Error(body.error ?? "Could not create mentor account.");

        const mentorId = body.id ?? "";
        onClose();
        const qs = new URLSearchParams({ invited: "1" });
        if (body.code) qs.set("code", body.code);
        if (body.tempPassword) qs.set("code", body.tempPassword);
        router.push(mentorId ? `/admin/mentors/${mentorId}?${qs.toString()}` : "/admin/mentors");
      } catch (err) {
