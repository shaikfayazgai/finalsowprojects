...
/** Inlined helper — no longer depends on mock service. */
function isMentorAdminSetupComplete(
  _mentor: Pick<MockAdminMentor, "id">,
  competency: MockCompetencyRow[],
): boolean {
  return competency.some(
    (r) =>
      Boolean(r.skillId) &&
...
/** Activity is not yet a backend endpoint — returns empty. */
const EMPTY_ACTIVITY: MockMentorActivityItem[] = [];
/** Audit events filtered by mentor are not yet a backend endpoint — returns empty. */
const EMPTY_AUDIT: MockAdminAuditEvent[] = [];
...
export function MentorDetailWorkspace() {
  ...
  async function resendCredentials() {
    if (!mentor) return;
    const targetEmail = mentor.email;
    setResending(true);
    try {
      const res = await fetch("/api/superadmin/users/resend-credentials", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: mentor.id, email: targetEmail }),
      });
      setToast(
        res.ok
          ? `Credentials re-sent to ${targetEmail}. They must reset the password on next sign-in.`
  ...
  // Activity feed and per-mentor audit trail are not yet backed by real endpoints.
  // They return empty arrays until those endpoints are wired.
  const activity = EMPTY_ACTIVITY;
  const auditEvents = EMPTY_AUDIT;
  ...
  return (
    <div className="space-y-5 pb-4 animate-fade-in">
        ...
        <div className="space-y-5">
          ...
          <SectionCard title="Actions" description="Lifecycle and routing for this mentor">
            <div className="flex flex-wrap gap-2">
              ...
              <button type="button" onClick={resendCredentials} disabled={resending} className={BTN_SECONDARY}>
      ...
      <MentorActionModals mentor={mentor} open={modal} onClose={() => setModal(null)} onSuccess={setToast} />
    </div>
  );
}
...
function AuditRow({ event: e }: { event: MockAdminAuditEvent }) {
  return (
    <li>
      <Link
