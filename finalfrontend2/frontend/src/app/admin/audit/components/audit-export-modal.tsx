 ...
 * Tenant list derived from real audit events via AuditEventsContext (no mock).
 */
...
export function AuditExportModal({ open, onClose }: AuditExportModalProps) {
  const { events: auditEvents } = useAuditEventsContext();
  // Derive unique tenant names from real fetched events
  const tenantOptions = React.useMemo(() => {
    const seen = new Map<string, string>();
    for (const e of auditEvents) {
      const key = e.tenantId ?? e.tenant ?? "";
      const label = e.tenant ?? key;
      if (key && !seen.has(key)) seen.set(key, label || key);
    }
