 ...
 *
 * Looks up event from AuditEventsContext (populated by AuditWorkspace).
 * Falls back to a standalone fetch when accessed directly (e.g. deep-link).
 *
...
export function AuditDetailWorkspace() {
  ...
  // Try context first (fast path when workspace loaded events); fall back to
  // a standalone fetch for direct deep-links where the context is empty.
  const [standaloneEvent, setStandaloneEvent] = React.useState<MockAdminAuditEvent | null | undefined>(undefined);
  const [standaloneLoading, setStandaloneLoading] = React.useState(false);
  const [standaloneError, setStandaloneError] = React.useState<string | null>(null);

  const contextEvent = findById(params.eventId);

  // If context has no events yet (direct navigation), fetch standalone
  React.useEffect(() => {
    if (contextEvent) return; // context hit — no need
    if (standaloneEvent !== undefined) return; // already fetched
    setStandaloneLoading(true);
    fetchAuditEvents({ limit: 500 })
      .then((result) => {
        const mapped = result.events.map(mapAuditViewEventToMock);
        const found = mapped.find((e) => e.id === params.eventId) ?? null;
        setStandaloneEvent(found);
      })
      .catch((err) => {
        const msg =
  ...
  }, [contextEvent, standaloneEvent, params.eventId]);

  const e: MockAdminAuditEvent | undefined = contextEvent ?? (standaloneEvent ?? undefined);
  const isLoading = standaloneLoading;
  ...
  if (isLoading) {
    return (
      <div className="space-y-5 pb-12 animate-fade-in">
        <Crumbs items={[{ label: "Audit log", href: "/admin/audit" }, { label: "Loading…" }]} />
        <p className="font-body text-[13px] text-text-secondary">Loading event…</p>
      </div>
    );
  }
  ...
  if (standaloneError) {
    return (
      <div className="space-y-5 pb-12 animate-fade-in">
        <Crumbs items={[{ label: "Audit log", href: "/admin/audit" }, { label: "Error" }]} />
        <p className="font-body text-[13px] text-text-secondary">{standaloneError}</p>
      </div>
    );
  }
