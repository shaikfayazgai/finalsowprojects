 ...
 * Step 2: enter the OTP code (verified against the backend)
 * Step 3: set a new password (POST /password/setup-after-otp) → done
 */
...
export default function ForgotPasswordPage() {
  ...
  async function sendOtp(e?: React.FormEvent) {
    ...
    try {
      ...
      setStep("code");
      setInfo(`We've emailed a 6-digit code to ${email}. It expires in 5 minutes.`);
    } catch {
    ...
    >
      ...
      {step === "code" && (
        <form onSubmit={verifyOtp} className="space-y-4">
          <AuthField label="Verification code" htmlFor="fp-code">
            <input id="fp-code" inputMode="numeric" autoComplete="one-time-code" value={code}
              onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 6))}
              placeholder="123456" className={authInputCls} required />
          </AuthField>
          <AuthSubmitButton disabled={code.trim().length < 4} loading={submitting}>
            Verify code
          </AuthSubmitButton>
          <button type="button" onClick={() => sendOtp()}
            className="font-body text-[13px] font-medium text-text-link hover:underline underline-offset-2">
            Resend code
          </button>
        </form>
      )}
      ...
      {step === "password" && (
        <form onSubmit={setNewPassword} className="space-y-4">
          <AuthField label="New password" htmlFor="fp-pw">
            <input id="fp-pw" type="password" autoComplete="new-password" value={password}
              onChange={(e) => setPassword(e.target.value)} placeholder="At least 8 characters"
              className={authInputCls} required />
          </AuthField>
          <AuthField label="Confirm password" htmlFor="fp-pw2">
            <input id="fp-pw2" type="password" autoComplete="new-password" value={confirm}
      ...
      {step === "done" && (
        <div className="space-y-3">
          ...
          <Link
            href="/auth/login"
            className="inline-flex h-11 w-full items-center justify-center rounded-xl bg-brand-emphasis font-body text-[14px] font-semibold text-white hover:opacity-90 transition-opacity"
          >
            Go to sign in
          </Link>
        </div>
