        ...
        {(mode === "upload" || mode === "compose") && (
          <Field label="Attachments" hint="PDF, DOCX up to 25 MB each">
            <div
              ...
              className={cn(
                "rounded-xl ring-1 px-4 py-5 transition-colors duration-150 backdrop-blur",
                "flex flex-col items-center justify-center gap-2 text-center",
                isDragging
                  ? "ring-2 ring-[var(--color-ai-border)] bg-[var(--color-ai-surface)]"
                  : "ring-white/55 bg-white/45",
              )}
            >
              <FileUp
                className={cn(
              ...
              <button
                ...
                className={cn(
                  "inline-flex items-center gap-1.5 h-7 px-2.5 rounded-xl",
                  "bg-white/55 border border-white/70 backdrop-blur text-foreground",
            ...
            {attachments.length > 0 && (
              <ul className="mt-2 space-y-1.5">
                {attachments.map((f, idx) => (
                  <li
                    key={idx}
                    className="flex items-center gap-2 rounded-xl bg-white/45 ring-1 ring-white/55 backdrop-blur px-3 py-2 font-body text-[12px] transition-colors duration-fast hover:bg-white/60"
                  >
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center h-6 w-6 rounded-md text-text-tertiary hover:bg-white/60 hover:text-foreground transition-colors duration-150"
                    >
