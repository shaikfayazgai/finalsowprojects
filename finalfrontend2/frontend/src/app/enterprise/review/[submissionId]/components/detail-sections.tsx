    ...
    <>
      ...
      <ul className="divide-y divide-white/60 -mx-5 sm:-mx-6">
        {ctx.criteria.map((c) => (
          <li
            key={c.id}
            className="flex items-center justify-between gap-3 px-5 sm:px-6 py-2.5 min-h-[44px] transition-colors duration-fast hover:bg-white/50"
          >
...
export function LineageSection({ ctx }: { ctx: AcceptanceDetailContext }) {
  return (
    <ol className="divide-y divide-white/60 -mx-5 sm:-mx-6">
      {ctx.lineage.map((step) => (
        <li
          key={step.stage}
          className="px-5 sm:px-6 py-2.5 min-h-[44px] flex items-center justify-between gap-3 transition-colors duration-fast hover:bg-white/50"
        >
          <span className="min-w-0">
    ...
    <>
      <ul className="divide-y divide-white/60 -mx-5 sm:-mx-6">
        {ctx.auditEvents.map((e, i) => (
          <li key={i} className="px-5 sm:px-6 py-2.5 flex items-center gap-3 flex-wrap transition-colors duration-fast hover:bg-white/50">
            <span className="font-mono text-[10.5px] text-text-tertiary tabular-nums shrink-0">
