...
export default function CompletedProjectsPage() {
  ...
  React.useEffect(() => {
    ...
    listEnterpriseProjects()
      .then(({ completed }) => {
        if (!cancelled) setProjects(completed);
      })
      .catch((err: unknown) => {
        if (!cancelled) setFetchError(err instanceof Error ? err.message : "Failed to load projects");
      })
      .finally(() => {
  ...
  return (
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      {loading ? (
        <div className={cn(GLASS_CARD, "overflow-hidden px-4 py-14 text-center")} style={GLASS_SHADOW}>
