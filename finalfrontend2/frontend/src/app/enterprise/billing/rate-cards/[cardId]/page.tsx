...
import type { RateCardDetail } from "@/lib/enterprise/mocks/rate-cards";
...
function backendToDetail(data: BackendRateCardsResponse, cardId: string): RateCardDetail | undefined {
  if (!data.rateCards) return undefined;
  const rc = data.rateCards;
  const currency = rc.currency ?? data.tenantCurrency ?? "INR";
  const rows: RateCardDetail["rows"] = [];
  if (rc.default != null) {
    rows.push({ role: "Default", skill: "Any", level: "L2", region: "India", rateMinorPerHour: rc.default });
  }
  for (const [seg, rate] of Object.entries(rc.bySegment ?? {})) {
    rows.push({
  ...
  return {
    ...
    currency,
    rows,
    bySegment: rc.bySegment
      ? {
          student: rc.bySegment["student"],
          women_workforce: rc.bySegment["women_workforce"],
          general_workforce: rc.bySegment["general_workforce"],
          internal: rc.bySegment["internal"],
        }
      : undefined,
  };
}
...
export default function RateCardDetailPage() {
  ...
  const card: RateCardDetail | undefined = React.useMemo(
    () => (rawData ? backendToDetail(rawData, cardId) : undefined),
    [rawData, cardId],
  );

  if (isLoading) {
    return <RateCardDetailSkeleton />;
  }
