 ...
 * Data comes from GET /api/enterprise/rate-cards-multi → backend /api/v1/enterprise/rate-cards.
 *
 * The backend stores a single tenant-level rate-card config. We present it
 * as one "active" card in the workspace. Creating/editing calls PUT on the
 * same endpoint. Multi-card list (by project/SOW scope) is roadmap Phase 2.
 */
...
interface BackendRateCardsResponse {
  tenantId: string;
...
function backendToSummaries(data: BackendRateCardsResponse): RateCardSummary[] {
  if (!data.rateCards) return [];
  const rc = data.rateCards;
  const bySegment = rc.bySegment ?? {};
  const rowCount = Object.keys(bySegment).length + (rc.default != null ? 1 : 0);
  return [
    {
      id: `rc-tenant-${data.tenantId}`,
      ...
      status: "active",
      currency: rc.currency ?? data.tenantCurrency ?? "INR",
    },
  ];
}

async function fetchRateCards(): Promise<BackendRateCardsResponse> {
  const res = await fetch("/api/enterprise/rate-cards-multi", { cache: "no-store" });
  if (!res.ok) throw new Error(`Failed to load rate cards: HTTP ${res.status}`);
  return res.json() as Promise<BackendRateCardsResponse>;
}
...
export function RateCardsWorkspace() {
  ...
  const all: RateCardSummary[] = React.useMemo(
    () => (rawData ? backendToSummaries(rawData) : []),
    [rawData],
  );
  ...
  return (
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      {error && (
        <div className="rounded-2xl border border-error-border bg-error-subtle px-4 py-3 font-body text-[12.5px] text-error-text">
          Failed to load rate cards — check backend connectivity.
        </div>
      )}
      ...
      <NewRateCardDrawer
        ...
        onSaved={() => {
          setDrawerOpen(false);
          void qc.invalidateQueries({ queryKey: ["enterprise", "rate-cards"] });
        }}
      />
    </div>
  );
  ...
  return (
    <div className="px-5 py-14 text-center">
      ...
      <p className="font-body text-[13px] font-semibold text-foreground">
        {isEmpty ? "No rate cards configured" : "No matches"}
      </p>
