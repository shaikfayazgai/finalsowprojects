    ...
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      >
        <Link
          ...
          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-lg hover:text-foreground hover:bg-white/55 transition-colors duration-fast"
        >
      ...
      {/* Rates table */}
      <section className={cn(GLASS_CARD, "overflow-hidden")} style={GLASS_SHADOW}>
        <header className="px-4 py-2.5 border-b border-white/55 flex items-center justify-between">
          ...
          <table className="w-full" aria-label="Rate rows">
            ...
            <tbody>
              {rows.map((row) => (
                <tr
                  key={row.id}
                  className="border-t border-white/55"
                >
                  <Td>
                    <input
                      value={row.role}
                  ...
                  <td className="px-2 py-2 align-middle">
                    <button
                      ...
                      className="h-7 w-7 inline-flex items-center justify-center rounded-lg text-text-tertiary hover:text-error-text hover:bg-white/55 disabled:opacity-40 disabled:cursor-not-allowed transition-colors duration-fast"
                    >
        ...
        <div className="flex flex-wrap items-center gap-2 px-4 py-2.5 border-t border-white/55 bg-white/40">
          <button
            type="button"
            onClick={addRow}
            className={cn(ghostBtnClass, "h-8 px-2.5 rounded-xl text-[12px]")}
          >
          ...
          <button
            ...
            disabled
            title="Phase 2"
            className={cn(ghostBtnClass, "h-8 px-2.5 rounded-xl text-[12px] text-text-tertiary cursor-not-allowed")}
          >
          ...
          <select
            ...
            title="Phase 2"
            style={GLASS_FIELD_STYLE}
            className="h-8 px-2.5 rounded-xl backdrop-blur-md font-body text-[12px] font-semibold text-text-tertiary cursor-not-allowed"
          >
      ...
      {submitError && (
        <p className="rounded-xl border border-error-border bg-error-subtle px-3 py-2 font-body text-[12px] text-error-text backdrop-blur">
          {submitError}
        </p>
      )}
...
const cellInputCls = cn(
  "w-full h-8 px-2 rounded-xl backdrop-blur-md",
