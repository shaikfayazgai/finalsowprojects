...
export default function InvoicesListPage() {
  ...
  const all: InvoiceSummary[] = data?.items ?? [];
  ...
  return (
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      {isLoading ? (
        <div className={cn(GLASS_CARD, "px-4 py-10")} style={GLASS_SHADOW}>
        ...
        <div className={cn(GLASS_CARD, "px-4 py-10 text-center")} style={GLASS_SHADOW}>
          <p className="font-body text-[13px] font-semibold text-foreground">
            {all.length === 0 ? "No invoices yet" : "No invoices match these filters"}
          </p>
          {all.length > 0 && (
            <button
              type="button"
