...
export function IntegrityPanel({ event }: { event: AuditViewEvent }) {
  return (
    <div
      className={cn(
        "rounded-2xl border px-4 py-3 flex flex-wrap items-center justify-between gap-3 backdrop-blur",
  ...
  return (
    <pre
      className={cn(
        ...
        "max-h-[420px] overflow-y-auto -mx-5 px-5 py-3 rounded-xl bg-foreground/[0.04] border border-white/60",
        className,
      )}
    >
      {text}
...
function DiffPane({
  ...
  return (
    <div
      className={cn(
        "rounded-xl border p-3 min-h-[140px] max-h-[420px] overflow-y-auto backdrop-blur",
        tone === "brand"
          ? "border-[var(--c-violet-400)]/40 bg-[var(--c-violet-500)]/[0.06]"
          : "border-white/60 bg-foreground/[0.04]",
      )}
    >
