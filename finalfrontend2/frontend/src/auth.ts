...
/**
 * Normalize the role string the backend returns to the canonical UserRole the
 * frontend (proxy.ts PORTAL_RULES, ROLE_HOME) expects. The FastAPI backend uses
 * "superadmin" (one word) and may use "freelancer"/"women"/"student" for the
 * contributor persona — map them all to canonical spellings so portal-matching
 * and post-login routing work.
 */
