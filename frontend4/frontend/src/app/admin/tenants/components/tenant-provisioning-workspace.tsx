...
export function TenantProvisioningWorkspace() {
  ...
  const adminCredentials = useAdminProvisioningStore((s) => s.adminCredentials);
  ...
  const cred = adminCredentials[tenantId];
        ...
        <section className="rounded-xl border border-stroke-subtle bg-surface overflow-hidden">
          <header className="px-5 pt-4 pb-3 border-b border-stroke-subtle flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-1 font-body text-[12.5px] text-text-secondary">
                A random temporary password was emailed to{" "}
                <span className="font-medium text-foreground">{inviteName}</span> ({inviteEmail}).
                They sign in at{" "}
                <code className="font-mono text-[11px] text-text-tertiary">/enterprise/login</code>{" "}
                and are forced to set a new password on first sign-in.
              </p>
            </div>
            <span
              className={cn(
                "inline-flex items-center gap-1 rounded-full px-2.5 py-1 font-body text-[10.5px] font-semibold shrink-0",
                cred?.emailSent
            ...
            >
              ...
              {cred?.emailSent ? "Credentials emailed" : cred ? "Email not configured" : "Provisioning…"}
            </span>
          </header>
          <div className="px-5 py-4 space-y-3">
            {/* Fallback: if the email couldn't be sent, surface the temp password. */}
            {cred && !cred.emailSent && cred.tempPassword && (
              <div className="rounded-lg border border-warning-border bg-warning-subtle/40 px-3 py-2.5">
                <p className="font-body text-[10.5px] font-bold uppercase tracking-[0.1em] text-warning-text mb-1">
                  Temporary password (email not sent — share securely)
                </p>
                <div className="flex items-center gap-2">
                  <code className="font-mono text-[13px] text-foreground">{cred.tempPassword}</code>
                  <button
                    type="button"
                    onClick={() => copyInvite(cred.tempPassword!)}
                    className="inline-flex items-center gap-1.5 h-8 px-3 rounded-md bg-brand text-on-brand font-body text-[12px] font-semibold hover:bg-brand-hover"
                  >
            ...
            <p className="font-body text-[11.5px] text-text-tertiary">
              No invite links — credential-based onboarding. Forced password reset on first sign-in · audited.
            </p>
          </div>
        </section>
      )}
