            ...
            <span className="inline-flex items-center gap-0.5 align-middle">
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  ...
                  className={cn(
                    "h-3.5 w-3.5",
                    i < ctx.mentorStars ? "text-warning-text fill-warning-text" : "text-text-disabled",
                  )}
                  strokeWidth={2}
                  aria-hidden
                />
...
export function CriteriaSection({ ctx }: { ctx: AcceptanceDetailContext }) {
  ...
  return (
    <>
      {unmet > 0 && (
        <p className="font-body text-[12px] text-warning-text font-medium mb-3 -mt-1">
      ...
      <ul className="divide-y divide-stroke-subtle -mx-5">
        {ctx.criteria.map((c) => (
          <li key={c.id} className="flex items-center justify-between gap-3 px-5 py-2.5 min-h-[44px]">
            ...
            <span
              className={cn(
                "font-body text-[11px] shrink-0 tabular-nums",
                c.met ? "text-success-text font-medium" : "text-warning-text font-medium",
              )}
            >
  ...
  return (
    <ul className="divide-y divide-stroke-subtle -mx-5">
      {ctx.artifacts.map((a) => {
        ...
        return (
          <li key={a.id} className="flex items-center justify-between gap-3 px-5 py-2.5 min-h-[44px]">
...
export function LineageSection({ ctx }: { ctx: AcceptanceDetailContext }) {
  return (
    <ol className="divide-y divide-stroke-subtle -mx-5">
      {ctx.lineage.map((step) => (
        <li key={step.stage} className="px-5 py-2.5 min-h-[44px] flex items-center justify-between gap-3">
          <span className="min-w-0">
            ...
            {step.href ? (
              <Link
                href={step.href}
                className="block font-body text-[13px] font-medium text-foreground hover:text-brand truncate transition-colors duration-fast"
              >
                {step.label}
              </Link>
            ) : (
  ...
  return (
    <>
      ...
      <p className="mt-3 font-body text-[12px]">
        <Link href={auditHref} className="text-text-link font-medium hover:underline underline-offset-2">
          Full audit trail
        </Link>
      </p>
    </>
