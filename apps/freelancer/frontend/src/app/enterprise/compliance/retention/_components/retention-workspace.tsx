...
export function RetentionWorkspace() {
  ...
  return (
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      <div className="rounded-xl border border-brand/20 bg-brand-subtle/15 px-4 py-3">
      ...
      {savedMsg && (
        <div className="rounded-xl border border-success-border bg-success-subtle px-4 py-3 font-body text-[12.5px] text-success-text flex items-center gap-2">
      ...
      <section className="rounded-xl border border-stroke-subtle bg-surface overflow-hidden">
        <div className="px-5 pt-4 pb-4 border-b border-stroke-subtle flex flex-wrap items-start justify-between gap-3">
          <div>
          ...
          <div className="flex flex-wrap items-center gap-2 shrink-0">
            ...
            <button
              ...
              className={cn(
                ...
                "bg-brand text-on-brand font-body text-[12.5px] font-semibold",
        ...
        {isLoading || !rows || !data ? (
          ...
          <ul className="divide-y divide-stroke-subtle">
            {RETENTION_ENTITIES.map((entity) => {
              ...
              return (
                <li key={entity} className="px-5 py-4">
                  <RetentionEntityRow
                    entity={entity}
                    floor={floor}
                    row={row}
      ...
      <DashboardSection title="Audit trail" description="Every save is tamper-evident">
        <p className="font-body text-[13px] text-text-secondary leading-relaxed -mx-5 px-5">
          Saving emits a signed{" "}
          <code className="font-mono text-[11px] text-foreground bg-bg-subtle px-1 py-0.5 rounded">
            retention_rules.update
          </code>{" "}
          event with a before/after diff.{" "}
          <Link
            href="/enterprise/audit?actionPrefix=retention_rules"
            className="text-text-link font-medium hover:underline underline-offset-2"
          >
            View in audit log
          </Link>
          .
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_minmax(280px,360px)] gap-4 lg:gap-8">
      ...
      <div className="space-y-3">
        <div
          ...
          className="inline-flex w-full p-0.5 rounded-lg bg-bg-subtle border border-stroke-subtle"
        >
          <ModeButton
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input
                ...
                className={cn(
                  "w-28 h-9 px-2.5 rounded-md border bg-surface",
                  "font-mono text-[13px] text-foreground tabular-nums",
                  "focus-visible:outline-none focus-visible:border-brand focus-visible:ring-2 focus-visible:ring-brand/20",
                  belowFloor
                    ? "border-error-border bg-error-subtle/30"
                    : "border-stroke",
                )}
              />
...
function ModeButton({
  ...
  return (
    <button
      ...
      aria-pressed={active}
      className={cn(
        "flex-1 inline-flex items-center justify-center gap-1.5 h-8 rounded-md",
