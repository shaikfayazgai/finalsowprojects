...
import type {
  ...
  MockAdminMentor, MockMentorPool, MockCompetencyRow,
  MockSkill,
  MockRubricTemplate,
  MockGovCase,
  MockKycCase,
...
export interface MentorsListResponse { items: MockAdminMentor[]; pools: MockMentorPool[] }
...
export interface MentorDetailResponse { mentor: MockAdminMentor; competency: MockCompetencyRow[]; pools: MockMentorPool[] }
...
export function fetchAdminPools(signal?: AbortSignal): Promise<{ items: MockMentorPool[]; mentors: MockAdminMentor[] }> {
  return getJSON(`/mentors/pools`, { signal });
}

export function fetchAdminPool(id: string, signal?: AbortSignal): Promise<{ pool: MockMentorPool; members: MockAdminMentor[] }> {
  return getJSON(`/mentors/pools/${encodeURIComponent(id)}`, { signal });
}
...
export function fetchUniversities(signal?: AbortSignal): Promise<{ items: MockUniversity[] }> {
  return getJSON(`/partnerships/universities`, { signal });
}
export function fetchUniversity(id: string, signal?: AbortSignal): Promise<{ partner: MockUniversity }> {
  return getJSON(`/partnerships/universities/${encodeURIComponent(id)}`, { signal });
}
