    ...
    <ol className="flex items-start">
      {INTAKE_STAGES.map((node, i) => {
        ...
        const nodeCls = isCurrent
          ? "bg-[var(--color-brand)] text-text-inverse ring-4 ring-[color-mix(in_oklab,var(--color-brand)_16%,transparent)]"
          : isDone
            ? "bg-[var(--color-brand)] text-text-inverse"
            : "bg-surface text-text-tertiary ring-1 ring-stroke-strong";
            ...
            {i < INTAKE_STAGES.length - 1 && (
              <span
                ...
                className={cn(
                  ...
                  connectorActive ? "bg-[var(--color-brand)]" : "bg-stroke-subtle",
                )}
              />
            )}
          </React.Fragment>
