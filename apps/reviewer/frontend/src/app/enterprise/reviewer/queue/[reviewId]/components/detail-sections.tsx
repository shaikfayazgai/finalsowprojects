  ...
  return (
    <ul className="divide-y divide-stroke-subtle -mx-5">
      {review.criteria.map((c) => (
        <li key={c.id} className="flex items-center justify-between gap-3 px-5 py-2.5 min-h-[44px]">
          ...
          <span className="inline-flex items-center gap-0.5 shrink-0" aria-label={`Mentor score ${c.mentorStars} of 5`}>
            {Array.from({ length: 5 }).map((_, i) => (
              <Star
                ...
                className={cn(
                  "h-3 w-3",
                  i < c.mentorStars ? "text-warning-text fill-warning-text" : "text-text-disabled",
                )}
                strokeWidth={2}
                aria-hidden
              />
    ...
    <>
      <ul className="divide-y divide-stroke-subtle -mx-5">
        {review.evidence.map((e) => {
          ...
          return (
            <li key={e.id} className="flex items-center justify-between gap-3 px-5 py-2.5 min-h-[44px]">
              <span className="flex items-center gap-2.5 min-w-0">
                <span className="h-7 w-7 rounded-md bg-bg-subtle flex items-center justify-center shrink-0">
              ...
              <span className="flex items-center gap-2 shrink-0">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 h-7 px-2 rounded-md border border-stroke bg-surface font-body text-[11px] font-semibold text-foreground hover:bg-surface-hover"
                >
...
export function CoverNoteSection({ review }: { review: MockReviewerItem }) {
  return (
    <p className="font-body text-[13px] text-foreground leading-relaxed -mx-5 px-5">
...
export function FileScanSection({ review }: { review: MockReviewerItem }) {
  ...
  return (
    <>
      ...
      <ul className="divide-y divide-stroke-subtle -mx-5">
        {scans.map((s) => (
          <li key={s.provider} className="px-5 py-2.5 flex items-start gap-3 min-h-[44px]">
      ...
      <p className="mt-3 font-body text-[11.5px] text-text-tertiary -mx-5 px-5">
  ...
  const color =
    status === "clean"
      ...
      : status === "warning"
        ? "text-warning-text"
        : "text-error-text";
  return <Icon className={cn("h-4 w-4 mt-0.5 shrink-0", color)} strokeWidth={2} aria-hidden />;
}
...
function ScanBadge({ status, compact = false }: { status: ScanResult["status"]; compact?: boolean }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center px-2 py-0.5 rounded-full font-mono font-bold uppercase tracking-[0.1em]",
...
export function VersionDiffSection({ review }: { review: MockReviewerItem }) {
  ...
  return (
    <div className="-mx-5">
      <div className="px-5 pb-2 flex items-center justify-end gap-2 font-mono text-[10.5px] tabular-nums">
        <span className="text-success-text inline-flex items-center gap-0.5">
          <Plus className="h-2.5 w-2.5" strokeWidth={2.5} aria-hidden />
          {adds}
        </span>
        <span className="text-error-text inline-flex items-center gap-0.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 border-t border-stroke-subtle divide-y md:divide-y-0 md:divide-x divide-stroke-subtle">
        <div>
          <p className="px-5 py-2 font-mono text-[10px] uppercase tracking-[0.1em] text-text-tertiary border-b border-stroke-subtle">
            Previous (v{review.round - 1})
          </p>
          <pre className="font-mono text-[11.5px] leading-relaxed p-5 overflow-x-auto">
            {lines
              .filter((l) => l.kind !== "add")
              .map((l, idx) => (
                <div
                  key={idx}
                  className={cn(
                    l.kind === "del" ? "bg-error-subtle/40 text-error-text" : "text-text-secondary",
                    "px-1.5 -mx-1.5 rounded-sm",
                  )}
                >
                  {l.kind === "del" ? "− " : "  "}
                  {l.text || " "}
            ...
            {lines
              ...
              .map((l, idx) => (
                <div
                  key={idx}
                  className={cn(
                    l.kind === "add" ? "bg-success-subtle/40 text-success-text" : "text-text-secondary",
                    "px-1.5 -mx-1.5 rounded-sm",
                  )}
                >
                  {l.kind === "add" ? "+ " : "  "}
