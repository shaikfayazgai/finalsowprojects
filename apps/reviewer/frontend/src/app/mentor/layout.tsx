...
function MentorShell({ children }: { children: React.ReactNode }) {
  ...
  // The /mentor/login is a public sign-in screen — render it bare (no shell,
  // no role guard that would bounce unauthenticated users to home).
  if (pathname.endsWith("/login")) {
    return <>{children}</>;
  }

  // Onboarding is a full-page flow — no sidebar, topbar, or dashboard chrome.
  if (isOnboarding) {
    return (
      <>
