...
/**
 * Shared auth shell — split-screen layout + form primitives.
 */
...
export const authInputCls =
  "block w-full h-11 px-3.5 rounded-lg border border-stroke bg-surface font-body text-[14px] text-foreground placeholder:text-text-tertiary focus:outline-none focus:ring-2 focus:ring-[color-mix(in_oklab,var(--color-brand)_28%,transparent)] focus:border-brand transition-all duration-fast";
...
export function AuthScreen({
  ...
  /** Optional content above the title (e.g. brand mark). */
  ...
  return (
    <div className="relative my-6">
      ...
      <p className="relative flex justify-center">
        <span className="bg-surface px-3 font-body text-[11px] font-semibold uppercase tracking-[0.08em] text-text-tertiary">
          {label}
        </span>
      </p>
    </div>
...
export function AuthSubmitButton({
  ...
  return (
    <button
      ...
      disabled={!active}
      className={cn(
        "w-full h-11 rounded-lg font-body text-[14px] font-semibold transition-all duration-fast mt-1",
        active
...
export function AuthAlert({
  ...
  const styles = {
    ...
    info: "border-brand/25 bg-brand-subtle/50 text-text-secondary",
  }[variant];

  return (
    <div
      role={variant === "error" ? "alert" : "status"}
      className={cn("rounded-lg border px-3.5 py-2.5 font-body text-[12.5px] mb-4", styles)}
    >
      {children}
    </div>
  );
...
export function AuthBackLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="inline-flex items-center gap-1.5 font-body text-[13px] font-semibold text-brand-emphasis hover:underline underline-offset-2 mt-5"
    >
      {children}
    </Link>
  );
...
function OAuthButton({
  ...
  return (
    <button
      ...
      className={cn(
        "inline-flex h-11 items-center justify-center gap-2.5 rounded-lg border border-stroke bg-surface",
        "font-body text-[13px] font-semibold text-foreground transition-all duration-fast",
        fullWidth ? "w-full px-4" : "flex-1 min-w-0 px-3",
        busy || disabled ? "opacity-60 cursor-not-allowed" : "hover:border-stroke-strong hover:bg-bg-subtle/40",
      )}
    >
      {busy ? (
        <span
