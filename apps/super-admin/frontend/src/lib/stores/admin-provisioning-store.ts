...
/** Result of provisioning the first enterprise admin (credential-based). */
...
interface AdminProvisioningState {
  ...
  /** tenantId → credential provisioning result for the first admin. */
  adminCredentials: Record<string, AdminCredentialRecord>;
  wizardDraft: TenantDraft | null;
  wizardStep: number;
  provisionTenant: (draft: TenantDraft, origin: string) => ProvisionedTenantRecord;
  setAdminProvisioned: (tenantId: string, rec: AdminCredentialRecord) => void;
  createInvite: (input: {
    email: string;
    name: string;
    roleKey: InviteRoleKey;
...
function buildProvisioningSteps(
  ...
  return [
    ...
    {
      id: "admin",
      label: `Admin credentials emailed (${adminEmail})`,
      state: "done",
      at: provisionedAt,
    },
    { id: "signin", label: "First admin sign-in (resets temp password)", state: "pending" },
...
export const useAdminProvisioningStore = create<AdminProvisioningState>()(
  persist(
    (set, get) => ({
      ...
      setAdminProvisioned: (tenantId, rec) =>
        set((s) => ({ adminCredentials: { ...s.adminCredentials, [tenantId]: rec } })),
...
/** Credential-based flow: mark the "First admin sign-in" step done for the
 *  tenant whose primary admin email matches the just-signed-in user. */
export function markAdminSignInByEmail(email: string | null | undefined) {
  if (!email) return;
  const target = email.trim().toLowerCase();
  const state = useAdminProvisioningStore.getState();
  const tenant = state.tenants.find((t) => t.adminEmail?.trim().toLowerCase() === target);
  if (tenant) {
    useAdminProvisioningStore.getState().markAdminSignIn(`mock-${tenant.id}`);
  }
}
