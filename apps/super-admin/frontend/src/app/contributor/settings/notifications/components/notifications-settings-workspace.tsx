...
import {
  useContributorSettingsStore,
  type NotifCategory,
} from "@/lib/stores/contributor-settings-store";
...
export function NotificationSettingsWorkspace() {
  const stored = useContributorSettingsStore((s) => s.notifications);
  const setNotifications = useContributorSettingsStore((s) => s.setNotifications);

  // Seed rows from the persisted store (falling back to INITIAL defaults).
  ...
  React.useEffect(() => {
    setRows(
      INITIAL.map((r) => ({
        ...r,
        prefs: stored[r.key as NotifCategory] ?? r.prefs,
      })),
    );
  }, [stored]);
  ...
  const onSave = () => {
    const next = rows.reduce(
      (acc, r) => {
        acc[r.key as NotifCategory] = r.prefs;
        return acc;
      },
      {} as Record<NotifCategory, Record<Channel, boolean>>,
    );
