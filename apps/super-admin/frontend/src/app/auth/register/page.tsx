...
function RegisterScreen() {
  ...
  // Women self-signup requires email OTP verification before submit.
  const isWomenSelfSignup = track === "women";
  const [otpSent, setOtpSent] = React.useState(false);
  const [otpCode, setOtpCode] = React.useState("");
  const [otpVerified, setOtpVerified] = React.useState(false);
  const [otpBusy, setOtpBusy] = React.useState(false);

  async function sendWomenOtp() {
    if (!email.includes("@")) { setError("Enter a valid email first."); return; }
    setOtpBusy(true); setError(null);
    try {
      await fetch("/api/auth/otp/send-email", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email.trim().toLowerCase() }),
      });
      setOtpSent(true);
    } catch { setError("Couldn't send the code. Try again."); }
    finally { setOtpBusy(false); }
  }
  async function verifyWomenOtp() {
    setOtpBusy(true); setError(null);
    try {
  ...
  const subtitle =
    track === "student"
      ...
      : track === "women_wf"
        ...
        : track === "women"
          ? "Join the Women in Tech track — your application is reviewed before access is granted."
    ...
    >
      <div className="space-y-5">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {isWomenSelfSignup && !otpVerified && (
            <div className="rounded-lg border border-stroke px-3.5 py-3 space-y-2.5">
              <p className="font-body text-[12.5px] text-text-secondary">
                Verify your email to submit your Women in Tech application.
              </p>
              {!otpSent ? (
                <button type="button" onClick={sendWomenOtp} disabled={otpBusy || !email.includes("@")}
                  className="h-9 px-4 rounded-lg text-[12.5px] font-semibold text-white bg-brand hover:bg-brand-hover disabled:opacity-60">
                ...
                <div className="flex items-center gap-2">
                  <input inputMode="numeric" maxLength={6} value={otpCode}
                    ...
                    className={authInputCls + " w-32 text-center font-mono tracking-[0.3em]"} />
                  <button type="button" onClick={verifyWomenOtp} disabled={otpBusy || otpCode.length !== 6}
                    className="h-9 px-3 rounded-lg text-[12px] font-semibold text-white bg-brand hover:bg-brand-hover disabled:opacity-60">
                    {otpBusy ? "Verifying…" : "Verify"}
                  </button>
                  <button type="button" onClick={sendWomenOtp} disabled={otpBusy}
                    className="text-[12px] text-text-tertiary hover:underline">Resend</button>
                </div>
              )}
            </div>
          )}
          {isWomenSelfSignup && otpVerified && (
            <p className="font-body text-[12px] text-success-text">Email verified ✓</p>
          )}

          <AuthSubmitButton disabled={!canSubmit} loading={submitting}>
            {isWomenSelfSignup ? "Submit application" : "Create account"}
          </AuthSubmitButton>
        </form>
