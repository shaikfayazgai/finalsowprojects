  ...
  return (
    <pre
      className={cn(
        ...
        "max-h-[420px] overflow-y-auto -mx-5 px-5 py-3 rounded-lg bg-bg-subtle/50 border border-stroke-subtle",
        className,
      )}
    >
      {text}
...
export function DiffSection({ event }: { event: AuditViewEvent }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 -mx-5 px-5">
        ...
        <li key={ev.id}>
          <Link
            ...
            className={cn(
              ...
              ev.id === currentId && "bg-brand-subtle/20",
            )}
          >
            ...
            <span
              className={cn(
                "shrink-0 inline-flex px-2 py-0.5 rounded-full font-body text-[10px] font-semibold capitalize",
                severityPillCls(ev.severity),
              )}
            >
              {ev.severity}
            </span>
      ...
      <span className="min-w-0 flex-1 text-right">
        {href ? (
          <Link
            href={href}
            className={cn(
              "font-body text-[13px] font-medium text-text-link hover:underline underline-offset-2",
  ...
  return (
    <div
      className={cn(
        ...
        tone === "brand"
          ? "border-brand/25 bg-brand-subtle/15"
          : "border-stroke-subtle bg-bg-subtle/40",
      )}
    >
