  ...
  return (
    ...
    >
      <section className="rounded-xl border border-stroke-subtle bg-surface overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-stroke-subtle">
          <div>
            <h2 className="font-body text-[15.5px] font-semibold text-foreground">Active endpoints</h2>
        ...
        <div
          aria-hidden
          className="hidden sm:grid grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_100px_120px_40px] gap-3 px-5 py-2 border-b border-stroke-subtle"
        >
...
function WebhookRow({
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "mt-2 sm:mt-0 inline-flex w-fit px-2 py-0.5 rounded-full font-body text-[10.5px] font-semibold",
          webhook.enabled ? "bg-success-subtle text-success-text" : "border border-stroke-subtle text-text-tertiary",
        )}
      >
      ...
      <div ref={menuRef} className="absolute top-3 right-4 sm:relative sm:top-auto sm:right-auto sm:flex sm:justify-end">
        ...
        {menuOpen && (
          <div className="absolute right-0 top-full mt-1 z-20 min-w-[148px] rounded-lg border border-stroke bg-surface shadow-md py-1">
...
function MenuItem({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full px-3 py-2 text-left font-body text-[12.5px] font-medium text-foreground"
    >
      {children}
    </button>
  );
    ...
    <Drawer
      ...
      footer={
        <div className="flex items-center justify-end gap-2 w-full">
          ...
          <button type="button" onClick={onSubmit} className={glassBtnPrimary}>
            Create webhook
          </button>
        </div>
      }
