...
import { MOCK_ADMIN_MENTORS, MOCK_MENTOR_COMPETENCY, MOCK_MENTOR_POOLS } from "@/mocks/admin/mentors";
...
export async function GET(_req: Request, { params }: { params: Promise<{ mentorId: string }> }) {
  ...
  return NextResponse.json({
    ...
    pools: MOCK_MENTOR_POOLS.filter((p) => mentor.pools.includes(p.id)),
  });
}
