  ...
  React.useEffect(() => {
    ...
    if (editCardId) {
      ...
      if (card) {
        ...
        setRows(
          card.rows.length > 0 ? card.rows.map(mockRowToFormRow) : [DEFAULT_ROW()],
        );
        return;
      }
    }
...
function ReviewPhase({
  ...
  return (
    <>
      ...
      <GlassCard className="p-3 flex items-start gap-2.5 border-brand/40 bg-brand/5">
        <CheckCircle2 className="h-4 w-4 text-brand shrink-0 mt-0.5" strokeWidth={2} aria-hidden />
...
function ImpactStat({
  ...
  return (
    <GlassCard
      className={cn(
        "px-3 py-2.5",
        emphasis && "border-brand/50 bg-brand/8",
      )}
    >
