...
import { provisionUser } from "@/lib/api/provision-user";
...
export function NewTenantWorkspace() {
  ...
  const setAdminProvisioned = useAdminProvisioningStore((s) => s.setAdminProvisioned);
  ...
  async function onProvision() {
    ...
    // Provision the FIRST enterprise admin for real: backend generates a random
    // temp password, emails the credentials, and flags must_change_password so
    // first sign-in forces a reset. (No invite URLs.)
    const [firstName, ...rest] = (d.adminName || "").trim().split(/\s+/);
    const result = await provisionUser({
      email: d.adminEmail,
      firstName: firstName || d.adminEmail.split("@")[0],
      lastName: rest.join(" "),
      role: "enterprise",
      tenantId: record.id,
    });
    if (result.ok) {
      setAdminProvisioned(record.id, {
        emailSent: !!result.emailSent,
        tempPassword: result.tempPassword,
      });
    } else {
      console.error("[tenant provision] admin credential email failed:", result.error);
    }
