...
import { fetchMentorReview, MentorApiError } from "@/lib/api/mentor-api";
  ...
  const onConfirmDecision = async (payload: object) => {
    ...
    try {
      await submitMentorReviewDecision(review.id, {
        ...
        qualityRatings: (p.ratings as Record<string, number> | undefined) ?? undefined,
        qualityRatingOverall: typeof p.ratingOverall === "number" ? p.ratingOverall : undefined,
      });
      setDecision(null);
            ...
            <p className="mt-1 font-mono text-[11px] text-text-tertiary tabular-nums inline-flex items-center gap-2 flex-wrap">
              ...
              {review.completionPct != null && (
                <>
                  ...
                  <span className="text-brand-subtle-text font-semibold">{review.completionPct}% complete</span>
                </>
              )}
            </p>
          )}
                  ...
                  <li key={e.id} className="flex items-center gap-3 px-1 py-2">
                    ...
                    {e.url ? (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1 h-7 px-2.5 rounded-md bg-surface border border-stroke font-body text-[11.5px] font-semibold text-foreground hover:bg-surface-hover"
                      >
                        <Eye className="h-3 w-3" strokeWidth={2} aria-hidden />
                        View
                      </a>
                    ) : (
                      <button type="button" disabled className="inline-flex items-center gap-1 h-7 px-2.5 rounded-md bg-surface border border-stroke font-body text-[11.5px] font-semibold text-text-tertiary opacity-50">
                        <Eye className="h-3 w-3" strokeWidth={2} aria-hidden />
                        View
                      </button>
                    )}
