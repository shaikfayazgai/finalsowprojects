...
export function AccountSettingsWorkspace() {
  ...
  const onSaveEmail = async () => {
    ...
    setEmailSaving(true);
    try {
      const res = await fetch("/api/contributor/settings/account/change-email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: email.trim().toLowerCase() }),
      });
      if (res.ok) {
        setEmailMsg("Email updated. You may need to verify the new address.");
        invalidateAccountAuth();
      } else {
        const body = (await res.json().catch(() => ({}))) as { detail?: string };
        setEmailMsg(body.detail ?? `Could not update email (${res.status}).`);
      }
    } catch {
      setEmailMsg("Could not update email. Please try again.");
    } finally {
