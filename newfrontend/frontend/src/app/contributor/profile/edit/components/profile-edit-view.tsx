...
export function ProfileEditView() {
  ...
  // Hydrate fields the persona hook doesn't carry (bio) and prefer the real
  // contributor_profiles values from the backend over session-derived defaults.
  React.useEffect(() => {
    let active = true;
    (async () => {
      try {
        const res = await fetch("/api/contributor/profile", { cache: "no-store" });
        ...
        /* keep session-derived defaults if the profile fetch fails */
      }
    })();
    return () => {
      active = false;
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const last_name = sp === -1 ? "" : trimmed.slice(sp + 1).trim();
      ...
      >
        <section className="rounded-xl border border-stroke-subtle bg-surface overflow-hidden min-w-0">
          ...
          <div className="p-5 space-y-5">
            ...
            {error ? (
              <div className="flex items-center gap-2 rounded-md bg-error-subtle border border-error-border px-3 py-2.5">
