...
export function SubmissionDetailView({ submissionId }: SubmissionDetailViewProps) {
  ...
  // Live delivery stage drives the routing label — contributor column of the
  // shared delivery-status matrix (single source of truth = the task).
  const routingLabel =
    (task?.deliveryStatus && deliveryLabel(task.deliveryStatus, "contributor")) ||
    ...
    <div className="pb-12 animate-fade-in">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px] xl:items-start">
        ...
        <div className="space-y-5 min-w-0">
          ...
          {task?.deliveryStatus === "paid" ? (
            <StatusBanner
              tone="success"
              title="Successfully completed — paid"
              description="Your payout has been released. Thank you for your work!"
              href={`/contributor/tasks/completed/${taskId}`}
              cta="View completed task"
              icon={Trophy}
            />
          ) : (task?.deliveryStatus === "payment_pending" || submission.status === "accepted") ? (
            <StatusBanner
              tone="success"
              title="QA review accepted · Payment pending"
              description="Your work passed QA review. The payout is being processed and will be released shortly."
              href={`/contributor/tasks/completed/${taskId}`}
              cta="View completed task"
              icon={Trophy}
            />
