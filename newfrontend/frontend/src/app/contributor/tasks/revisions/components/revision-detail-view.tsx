...
import { getRevisionTaskDetail } from "@/lib/api/contributor-task-views";
import {
  useSubmitSubmission,
  useUpdateSubmission,
...
export function RevisionDetailView({ taskId }: RevisionDetailViewProps) {
  ...
  // Ready to resubmit when every itemised correction is ticked — and ALSO when the
  // mentor sent the rework with a free-text comment and no checklist (0 items).
  // (Was `correctionsTotal > 0 && …`, which left Resubmit permanently disabled on
  // comment-only reworks → "how do I submit, there's no button".)
  const allAddressed = addressed.every(Boolean);
  const pct =
    correctionsTotal > 0 ? Math.round((addressedCount / correctionsTotal) * 100) : 100;
