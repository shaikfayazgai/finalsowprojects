...
export function countWorkLanes(items: ContributorTaskSummary[]) {
  ...
  for (const t of items) {
    if (isSubmissionLaneTask(t)) {
      // Handed off, waiting on the mentor — not actionable by the contributor.
      submissions++;
    } else if (isRevisionLaneTask(t)) {
      // A revision is still ASSIGNED work the contributor must act on, so it counts
      // toward Assigned (in-progress bucket) AND is tracked as a revision.
      assigned++;
      inProgress++;
      revisions++;
