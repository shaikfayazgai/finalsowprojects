...
import { listMyPayoutMethods } from "@/lib/api/contributor-payouts";
...
export default function WithdrawSuccessPage() {
  ...
  const [methods, setMethods] = React.useState<DisplayMethod[] | null>(null);
  React.useEffect(() => {
    let active = true;
    listMyPayoutMethods()
      .then((r) => {
        ...
        setMethods(
          r.items.map((m) => ({
            id: m.id,
            label: m.nickname ?? m.kind ?? "Payout method",
            primary: m.isDefault,
          })),
        );
      })
