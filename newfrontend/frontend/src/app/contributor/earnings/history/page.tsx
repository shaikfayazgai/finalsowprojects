 ...
 * Paginated table backed by the real payouts API (/api/payouts via listMyPayouts).
 */

import * as React from "react";
import { AlertCircle } from "lucide-react";
import { listMyPayouts } from "@/lib/api/contributor-payouts";
...
export default function ContributorEarningsHistoryPage() {
  ...
  React.useEffect(() => {
    ...
    listMyPayouts()
      .then((res) => {
        if (!active) return;
        const items: HistoryRow[] = res.items.map((p) => ({
          id: p.id,
          taskTitle: p.taskDefinitionId ? `Task ${p.taskDefinitionId}` : "Payout",
          project: "—",
