...
export default function ExportEarningsPage() {
  ...
  const onGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const res = await fetch("/api/contributor/earnings/export", { cache: "no-store" });
      if (!res.ok) throw new Error(`Export failed (${res.status})`);
      const blob = await res.blob();
      const ts = new Date().toISOString().slice(0, 10);
      const filename = `glimmora-earnings-${range}-${ts}.csv`;
      ...
      setResultMsg(`Downloaded ${filename}`);
    } catch (err) {
      setResultMsg(err instanceof Error ? err.message : "Could not generate export.");
    } finally {
      setGenerating(false);
    }
  };
