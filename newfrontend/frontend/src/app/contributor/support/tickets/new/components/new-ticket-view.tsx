...
import { getTask } from "@/lib/api/contributor-tasks";
...
export function NewTicketView({ taskId }: NewTicketViewProps) {
  ...
  const [task, setTask] = React.useState<{ id: string; title: string; status: string } | null>(null);
  ...
  React.useEffect(() => {
    ...
    getTask(taskId)
      .then((res) => {
        ...
        setTask({ id: res.task.id, title: res.task.title, status: res.task.status });
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/contributor/support/tickets", {
        method: "POST",
      ...
      const ref = row?.id != null ? `TKT-${row.id}` : `TKT-${Date.now().toString().slice(-6)}`;
      setSubmittedRef(ref);
