...
import { type AdminAuditSeverity, type MockAdminAuditEvent } from "@/mocks/admin/audit";
import { useAdminAuditLog } from "@/lib/hooks/use-admin-audit";
...
export function AuditWorkspace() {
  ...
  const { events: auditEvents } = useAdminAuditLog();

  const scopedEvents = React.useMemo(
    () => filterAuditEventsForRole(role, auditEvents),
    [role, auditEvents],
  );
  ...
  const roleScoped = scopedEvents.length !== auditEvents.length;
