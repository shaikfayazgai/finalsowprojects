...
export function AdminDashboardWorkspace() {
  ...
  // Real KPIs/pipeline/recent from the super-admin backend (live DB counts:
  // tenants, mentors, active SOWs, governance open, KYC pending, audit recents).
  // The tenant list refines the tenant/active counts to match /admin/tenants.
  const { data: dash } = useAdminDashboard();
  const { tenants: liveTenants } = useAdminTenants();
  const d = React.useMemo(() => {
    if (!liveTenants) return dash;
    const activeTenants = liveTenants.filter((t) => t.status === "active").length;
    return {
      ...dash,
      kpis: { ...dash.kpis, tenants: liveTenants.length },
      pipeline: { ...dash.pipeline, tenantsActive: activeTenants },
    };
  }, [dash, liveTenants]);
