...
export function AnalyticsWorkspace() {
  // Real analytics computed from this tenant's delivery data (no mock).
  const { data: overview, isLoading } = useQuery({
    queryKey: ["enterprise", "analytics", "overview"],
    queryFn: async (): Promise<AnalyticsOverview> => {
      const res = await fetch("/api/enterprise/analytics/overview", { cache: "no-store" });
  ...
  if (isLoading || !overview) {
    return (
      <div className="p-6 font-body text-[13px] text-text-tertiary animate-fade-in">
        Loading analytics…
      </div>
    );
  }
