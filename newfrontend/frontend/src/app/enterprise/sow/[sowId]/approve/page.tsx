...
function approveStageDescription(stage: SowStage): string {
  switch (stage) {
    ...
    case "tenant_admin":
      return "Tenant admin sign-off — sign with your name to release this SOW to the Super admin.";
    case "platform":
      return "Super admin approval — final signed gate before delivery.";
    default:
...
const STAGE_CHECKLIST: Record<SowStage, string[]> = {
  ...
  tenant_admin: [
    "Security, Finance, and Legal gates all passed",
    "Scope, budget, and timeline confirmed for the tenant",
    "Sign with your name to authorise this SOW",
  ],
  platform: [
    "All enterprise gates (Security, Finance, Legal, Tenant admin) passed",
...
const SUBMIT_CHECKLIST = [
  ...
  "Gates run next: Security → Finance → Legal → Tenant admin, then the Super admin signs off",
];
...
export default function SowApprovePage() {
  ...
  const onSubmitDecision = async () => {
    ...
    // The Tenant-admin gate requires a signature (typed name) to approve.
    if (decision === "approve" && currentStage === "tenant_admin" && trimmedSignature.length < 2) {
      setActionError("Sign off by typing your full name to approve as tenant admin.");
      return;
    }
    try {
      if (decision === "approve") {
        await approve.mutateAsync({
          ...
          signature: currentStage === "tenant_admin" ? trimmedSignature : undefined,
        });
    ...
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      {isDraft && (
        <Section
          ...
          description="Starts approval: Security → Finance → Legal → Tenant admin, then Super admin."
        >
      ...
      {awaitingGate && currentStage && (
        <WaitBanner
          ...
          body={
            isOwner
              ? "You submitted this SOW. Each enterprise internal gate is approved by its reviewer (Security, Finance, Legal) — you cannot approve your own submission."
      ...
      {platformWait && (
        <WaitBanner
          title="Awaiting Super admin"
          body={
            isOwner
              ? "All enterprise gates (incl. tenant-admin sign-off) have passed. The Super admin is performing the final approval."
              : "This SOW is with the Super admin for the final approval gate."
          }
        />
      )}
      ...
      <Section
        ...
        description="Security → Finance → Legal → Tenant admin → Super admin"
      >
        <div className="px-5 sm:px-6 py-5">
          <ApprovalStepper
...
function DecisionPanel({
  ...
  // Tenant-admin gate requires a typed-name signature to approve.
  const needsSignature = stage === "tenant_admin" && decision === "approve";
    ...
    <div className="space-y-4">
      ...
      {needsSignature && (
        <div>
          ...
          <input
            ...
            disabled={isPending}
            className={cn(
              "w-full h-10 px-3 rounded-lg border border-stroke-subtle bg-surface",
          ...
          <p className="mt-1.5 font-body text-[11.5px] text-text-tertiary">
            Recorded as the tenant admin&apos;s signature in the audit log.
          </p>
        </div>
      )}
      ...
      <div className="flex items-center justify-end gap-2 border-t border-stroke-subtle -mx-5 sm:-mx-6 px-5 sm:px-6 pt-4">
        ...
        <button
          type="button"
          onClick={onSubmit}
          disabled={isPending || (needsSignature && signature.trim().length < 2)}
