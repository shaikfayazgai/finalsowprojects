...
/** A provisioned (active/archived) decomposition plan IS a delivery project. */
function planToProjectSummary(plan: PlanSummary): ProjectSummary {
  // Live delivery rollup from the backend (task-status based). `progress` is a
  // 0..1 fraction here (the row renders it as progress*100).
  const pct = plan.delivery?.progressPct ?? 0;
  const fullyComplete = plan.delivery?.paymentPhase === "payment_completed" || pct >= 100;
  return {
    id: plan.id,
    name: plan.sowTitle ?? `Plan ${plan.id.slice(0, 8)}`,
    sponsor: "",
    pmo: plan.createdBy ?? "—",
    startedAt: plan.activatedAt ?? plan.approvedAt ?? plan.createdAt,
    dueAt: plan.updatedAt,
    progress: pct / 100,
    health: "on_track",
    completedAt: fullyComplete ? (plan.archivedAt ?? plan.updatedAt) : (plan.archivedAt ?? undefined),
    sowId: plan.sowId,
    planId: plan.id,
  };
}
...
export default function ProjectsListPage() {
  ...
  // Real provisioned projects = active / archived decomposition plans (live DB).
  const activeQuery = usePlanList({ status: ["active"], limit: 200 });
  const completedQuery = usePlanList({ status: ["archived"], limit: 200 });
  const active = React.useMemo(
    () => (activeQuery.data?.items ?? []).map(planToProjectSummary),
    [activeQuery.data],
  );
  const completed = React.useMemo(
    () => (completedQuery.data?.items ?? []).map(planToProjectSummary),
    [completedQuery.data],
  );
  const loadingProjects = activeQuery.isLoading || completedQuery.isLoading;
              ...
              <table className="w-full min-w-[760px] border-collapse">
                ...
                <tbody>
                  {pageRows.map((project) => (
                    <ProjectRow key={project.id} project={project} completedView={completedView} onOpen={() => router.push(`/enterprise/decomposition/${project.id}`)} />
                  ))}
                </tbody>
              </table>
            </div>
...
function ProjectRow({ project, completedView, onOpen }: { project: ProjectSummary; completedView: boolean; onOpen: () => void }) {
  ...
  const href = `/enterprise/decomposition/${project.id}`;
