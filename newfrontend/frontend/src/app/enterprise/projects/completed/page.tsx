...
export default function CompletedProjectsPage() {
  ...
  // Completed projects = archived decomposition plans (live DB).
  const archivedQ = usePlanList({ status: ["archived"], limit: 200 });
  const projects = React.useMemo(
    () =>
      (archivedQ.data?.items ?? []).map((plan) => ({
        id: plan.id,
        name: plan.sowTitle ?? `Plan ${plan.id.slice(0, 8)}`,
        sponsor: "—",
        startedAt: plan.activatedAt ?? plan.approvedAt ?? plan.createdAt,
        completedAt: plan.archivedAt ?? plan.updatedAt,
      })),
    [archivedQ.data],
  );
      ...
      {projects.length === 0 ? (
        ...
        <div className={cn(DASH_CARD, "overflow-hidden")}>
          <table className="w-full" aria-label="Completed projects">
            ...
            <tbody>
              {projects.map((p) => {
                const href = `/enterprise/decomposition/${p.id}`;
