...
import { usePlan, useSubmitPlan } from "@/lib/hooks/use-decomposition-v2";
...
export default function DecompositionApprovalPage() {
  ...
  const submit = useSubmitPlan(planId);
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setActionError(e instanceof Error ? e.message : "Failed to submit to Glimmora");
    }
  };
  ...
  return (
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      <header className={cn(DASH_CARD, "p-5 flex items-start gap-4")}>
        ...
        <div className="min-w-0">
          <p className="font-body text-[10.5px] font-bold uppercase tracking-[0.14em] text-text-tertiary mb-1.5">
            Submit to Glimmora · Version {plan.version}
          </p>
      ...
      {plan.revisionNote && plan.status === "draft" && (
        <div className="rounded-lg border px-4 py-3" style={{ background: TONE.error.soft, borderColor: TONE.error.border }}>
          <p className="font-body text-[11px] font-bold uppercase tracking-wide mb-1" style={{ color: TONE.error.text }}>
            Returned by Glimmora for revision
          </p>
          <p className="font-body text-[12.5px] text-foreground">{plan.revisionNote}</p>
        </div>
      )}

      {canDecide && (
        <Section
          title="Submit to Glimmora"
          description="Submitting sends this plan to Glimmora to price the contributor work and approve. Pricing is set by Glimmora and is not shown to you."
        >
          <div className="px-5 sm:px-6 py-5 space-y-3">
            <ul className="space-y-1.5 font-body text-[12.5px] text-text-secondary">
              {APPROVAL_CHECKLIST.map((c) => (
                <li key={c} className="flex items-start gap-2">
                  <Check className="h-3.5 w-3.5 mt-0.5 shrink-0 text-text-tertiary" strokeWidth={2.5} aria-hidden />
                  {c}
                </li>
              ))}
            </ul>
            {actionError && <p className="font-body text-[12px]" style={{ color: TONE.error.text }}>{actionError}</p>}
            <button
              type="button"
              onClick={onSubmit}
              disabled={isPending}
              className="inline-flex items-center justify-center gap-2 rounded-md bg-brand px-4 py-2 font-body text-[13px] font-semibold text-on-brand hover:bg-brand-hover disabled:opacity-60"
            >
              <CheckCircle2 className="h-4 w-4" strokeWidth={2} aria-hidden />
              {isPending ? "Submitting…" : "Submit to Glimmora"}
            </button>
          </div>
        </Section>
      )}
...
function MilestonePreviewRow({ milestone, plan }: { milestone: MilestoneDetail; plan: PlanDetail }) {
  ...
  return (
    <li className="transition-colors duration-fast">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        disabled={tasks.length === 0}
        className="flex w-full items-center justify-between gap-4 px-5 sm:px-6 py-2.5 min-h-[44px] text-left hover:bg-bg-subtle disabled:hover:bg-transparent disabled:cursor-default"
        aria-expanded={open}
      >
        <span className="flex items-center gap-2 font-body text-[13px] font-medium text-foreground truncate min-w-0">
          {tasks.length > 0 ? (
            <ChevronDown className={cn("h-3.5 w-3.5 shrink-0 text-text-tertiary transition-transform", open && "rotate-180")} aria-hidden />
          ) : (
            <span className="w-3.5 shrink-0" aria-hidden />
          )}
          <span className="font-mono text-[10px] text-text-tertiary">M{milestone.order}</span>
      ...
      {open && tasks.length > 0 && (
        <ul className="border-t border-stroke-subtle bg-bg-subtle/30">
          {tasks.map((t) => (
            <li key={t.id} className="flex items-start justify-between gap-4 pl-12 pr-5 sm:pr-6 py-2">
              <span className="min-w-0">
                <span className="block font-body text-[12.5px] text-foreground truncate">{t.title}</span>
                {typeof t.description === "string" && t.description ? (
                  <span className="mt-0.5 block font-body text-[11px] text-text-secondary leading-relaxed line-clamp-2">{t.description}</span>
                ) : null}
                {t.requiredSkills.length > 0 ? (
                  <span className="mt-0.5 flex flex-wrap gap-1">
                    {t.requiredSkills.map((s) => (
                      <span key={s} className="rounded bg-surface px-1.5 py-0.5 text-[10px] text-text-tertiary">{s}</span>
                    ))}
                  </span>
                ) : (
                  <span className="font-body text-[10.5px]" style={{ color: TONE.warning.text }}>no skill tags</span>
                )}
                  ...
                  <span className="mt-1 flex flex-wrap items-center gap-1.5">
                    ...
                    {t.attachments.map((a) => (
                      <a key={a.url} href={a.url} target="_blank" rel="noopener noreferrer"
                         className="rounded border border-stroke-subtle bg-surface px-1.5 py-0.5 text-[10px] text-text-link hover:underline max-w-[160px] truncate">
                        {a.name}
                      </a>
                    ))}
                  </span>
