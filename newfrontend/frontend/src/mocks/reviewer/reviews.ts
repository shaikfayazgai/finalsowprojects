...
export interface MockReviewerEvidence {
  ...
  sizeBytes: number;
  /** Viewable URL (GitHub deliverable or uploaded file). */
  url?: string;
}
...
export interface MockReviewerItem {
  ...
  taskTitle: string;
  taskSubtitle: string;
  /** Role-shared task code + readable submission id (same as contributor/mentor). */
  taskRef?: string;
  submissionRef?: string;
  /** Task brief + acceptance criteria + the contributor's GitHub deliverable link. */
  brief?: string;
  acceptanceCriteria?: string | null;
  /** Task's required skills (no commercials) — shown on the Task brief tab. */
  requiredSkills?: string[];
  githubUrl?: string | null;
  /** Contributor's self-assessed completion % (0-100). */
