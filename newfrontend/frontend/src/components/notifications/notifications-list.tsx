...
export function NotificationsList({
  ...
  // Category buckets (Action / Update / Payment / Complaint / Security).
  ...
  const shown = cat === "all" ? notifications : notifications.filter((n) => catOf(n) === cat);
  const catCount = (c: string) => (c === "all" ? notifications.length : notifications.filter((n) => catOf(n) === c).length);
  ...
  return (
    <div className="space-y-4">
      ...
      <nav aria-label="Filter by category" className="flex flex-wrap items-center gap-1.5">
        {CATS.map((c) => (
          <TabButton
            key={c}
            active={cat === c}
            onClick={() => setCat(c)}
            count={catCount(c)}
            tone={c === "security" && catCount(c) > 0 ? "brand" : "neutral"}
          >
