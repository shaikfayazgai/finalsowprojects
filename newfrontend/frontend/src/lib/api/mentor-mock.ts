...
export function adjudicateMentorEscalation(
  id: string,
  input: { decision: string; reason: string; coach?: boolean },
  signal?: AbortSignal,
): Promise<EscalationDetailResponse> {
  return getJSON<EscalationDetailResponse>(`/escalations/${encodeURIComponent(id)}/adjudicate`, {
    method: "POST",
...
// ── Mentorship stats (profile tile) ───────────────────────────────────────

export interface MentorshipStatsResponse { sessionsHeld: number; activeMentees: number }

export function fetchMentorMentorshipStats(signal?: AbortSignal): Promise<MentorshipStatsResponse> {
  return getJSON<MentorshipStatsResponse>("/mentorship-stats", { signal });
}
