...
export async function submitMentorReviewDecision(
  ...
  // REAL backend: record the decision on the mentor_reviews row. On "accept"
  // the backend hands the work off to the reviewer queue (reviewer_assignments).
  const res = await fetchInternal(
    `/api/mentor/submissions/${encodeURIComponent(reviewId)}/decide`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
  ...
  if (!res.ok) {
    ...
    throw new Error(body.error ?? body.detail ?? "Could not submit decision.");
  }
}
