...
const ROUTE_ACCESS: Array<{ prefix: string; roles: EnterpriseRole[] | "all" }> = [
  ...
  { prefix: "/enterprise/settings/team", roles: ["admin"] },
...
export function canApproveSowStage(roles: EnterpriseRole[], stage: SowStage): boolean {
  switch (stage) {
    ...
    case "tenant_admin":
      // Final enterprise sign-off — the tenant's workspace admin signs.
      return roles.includes("admin");
    case "platform":
    default:
      return false;
...
export function canActOnEnterpriseStage(
  ...
  // Locked SOW flow: the enterprise workspace admin operates all internal
  // governance gates (Finance/Security/Legal are admin approve-buttons, not
  // separate reviewer users), so the admin may sign even a SOW they uploaded.
  if (roles.includes("admin")) return true;
  // Four-eyes for dedicated reviewer personas: never your own submission.
