...
// Seed cleared to empty — the decomposition list starts with NO fabricated
// plans so dashboards/pages show real (backend-created) plans or an empty
// state, never mock `plan-acme-*` data. (`.slice(0,0)` keeps the sample plan
// definitions referenced for when a real backend wiring needs their shape.)
const SEED: PlanDetail[] = [PLAN_1, PLAN_2, PLAN_5, PLAN_7, PLAN_10, PLAN_OLD].slice(0, 0);
...
/** Hard-delete a plan (tombstone in the overlay). The source SOW returns to the
 *  "Awaiting decomposition" queue so it can be decomposed again. */
export function deletePlanMock(id: string): void {
  overlay.remove(id);
}
