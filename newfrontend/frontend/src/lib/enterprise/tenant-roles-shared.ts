...
/**
 * Raw account-level roles (login_accounts.role / session) that denote the tenant
 * admin but aren't tenant sub-roles. Mapping them to `admin` gives the multi-role
 * enterprise owner a STABLE persona from /api/me — otherwise it normalizes to []
 * and the dashboard falls back to a racy guess that can flip to "reviewer" while
 * switching portals.
 */
const ENTERPRISE_ROLE_ALIASES: Record<string, EnterpriseRole> = {
  enterprise: "admin",
...
/** Accepts `finance`, `ent.finance`, `enterprise` (→admin), or unknown → null. */
export function normalizeEnterpriseRole(code: string): EnterpriseRole | null {
  const raw = code.startsWith("ent.") ? code.slice(4) : code;
  if ((ENTERPRISE_ROLES as readonly string[]).includes(raw)) {
    return raw as EnterpriseRole;
  }
  return ENTERPRISE_ROLE_ALIASES[raw] ?? null;
}
