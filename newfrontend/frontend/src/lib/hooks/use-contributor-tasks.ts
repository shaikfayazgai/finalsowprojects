...
export function useMyTasks(params: { status?: string | string[]; limit?: number } = {}) {
  ...
  return useQuery({
    ...
    // Keep the work lists (Assigned/Submissions/Revisions/Completed all derive from
    // this) fresh — a task assigned/decided while the page is open shows within
    // seconds instead of looking "empty" until a manual reload.
...
export function useRevisionsQueue() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RevisionRow[]> => {
      ...
      return revisionsFromTasks(items);
    },
    refetchInterval: 15_000,
    refetchOnWindowFocus: true,
    staleTime: 0,
...
export function useCompletedTasks() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<{ items: CompletedRow[]; totalEarnedMinor: number }> => {
      ...
      return completedFromTasks(items);
    },
    refetchInterval: 15_000,
    refetchOnWindowFocus: true,
    staleTime: 0,
