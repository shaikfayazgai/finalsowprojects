...
/**
 * Normalize backend role spellings to the canonical proxy/portal enum.
 * The backend emits e.g. "superadmin" / "freelancer" / "women" / "student";
 * the route guards expect "super_admin" / "contributor".
 */
export function normalizeRole(role: string | null | undefined): UserRole {
  const r = (role ?? "").toLowerCase().trim();
  if (r === "superadmin" || r === "super_admin" || r === "plat.super_admin" || r === "plat.admin") return "super_admin";
  if (r === "admin" || r.startsWith("admin")) return "admin";
  if (r === "enterprise" || r.startsWith("ent")) return "enterprise";
  if (r === "reviewer" || r.startsWith("reviewer") || r.startsWith("ent.reviewer")) return "reviewer";
  if (r === "mentor" || r.startsWith("mentor")) return "mentor";
  if (r === "freelancer" || r === "women" || r === "student" || r === "contributor") return "contributor";
  return (r || "contributor") as UserRole;
}
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      if (user) {
        ...
        token.role = normalizeRole(u.role as string | undefined);
      ...
      if (shouldRefresh) {
        try {
          ...
          // unavailable auth backend) must NOT nuke a still-valid token — doing
          // so left the session logged-in but token-less, which is what produced
