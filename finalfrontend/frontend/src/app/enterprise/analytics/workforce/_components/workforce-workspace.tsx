...
export function WorkforceWorkspace() {
  ...
  return (
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      <SectionCard title="Workforce summary" description={`${m.totalContributors} active contributors in window`}>
      ...
      <SectionCard
        ...
        description={`Top ${m.topSkills.length} skills by contributor count`}
      >
      ...
      >
        {m.skillGaps.length === 0 ? (
          <p className="font-body text-[13px] text-text-tertiary italic px-5 sm:px-6 py-5">
          ...
          <ul className="divide-y divide-white/60">
            {m.skillGaps.map((g, i) => (
              <li
                ...
                className="flex items-center justify-between gap-4 px-5 sm:px-6 py-2.5 min-h-[44px] bg-warning-subtle/10"
              >
      ...
      <SectionCard
        ...
        description={`Tasks completed · ${m.throughput.length} weeks in window`}
      >
              ...
              return (
                <div key={b.weekStart} className="flex-1 flex flex-col items-stretch min-w-0 group">
                  <div className="flex-1 flex items-end">
                    <div
                      className="w-full rounded-t-md opacity-90 group-hover:opacity-100 transition-opacity duration-fast"
      ...
      <SectionCard title="Acceptance rate" description="First-pass acceptance across all submissions">
        <div className="flex flex-wrap items-baseline gap-x-4 gap-y-2 px-5 sm:px-6 py-5">
          <p className="font-display text-[28px] font-semibold text-foreground tabular-nums">
            {m.acceptanceRatePct}%
          </p>
  ...
  return (
    <div>
      ...
      <dd
        ...
        style={!alert && highlight ? ACCENT_TEXT : undefined}
      >
        {value}
      </dd>
