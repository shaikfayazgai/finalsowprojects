...
import { GLASS_FIELD_STYLE, ghostBtnClass, primaryBtnClass, primaryStyle } from "@/app/admin/_shell/aurora-ui";
  ...
  return (
    ...
    >
      <div className="p-4 space-y-4">
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? "text-white" : "border border-white/70 bg-white/55 backdrop-blur text-text-secondary hover:bg-white/75 hover:text-foreground",
                  )}
        ...
        <div className="rounded-xl border border-white/55 bg-white/50 backdrop-blur px-3 py-2">
        ...
        <ol className="space-y-1.5">
          {lines.map((l, i) => (
            <li key={l} className="flex items-center gap-2 font-body text-[12px]">
              {i <= active ? <CheckCircle2 className="h-3.5 w-3.5 text-success" strokeWidth={2.5} aria-hidden /> : <span aria-hidden className="h-3.5 w-3.5 inline-block rounded-full border border-white/70" />}
  ...
  return (
    ...
    >
      <div className="px-5 sm:px-6 py-5 space-y-5">
        {/* Generation banner */}
        <div className="rounded-xl border border-white/55 bg-white/50 backdrop-blur px-3 py-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11.5px] font-body">
        ...
        <section>
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
            {data.guardrails.map((g) => (
              <li key={g.n} className="rounded-xl border border-white/55 bg-white/55 backdrop-blur px-2.5 py-1.5 flex items-start gap-2">
                {g.status === "pass"
        ...
        <section>
          ...
          <ul className="space-y-1.5">
            {data.deliverables.map((d, i) => (
              <li key={d.id} className="rounded-xl border border-white/55 bg-white/55 backdrop-blur px-2.5 py-1.5 flex items-center gap-2 transition-colors duration-fast hover:bg-white/70">
                ...
                <button type="button" onClick={() => patch("deliverables", data.deliverables.filter((_, j) => j !== i))} aria-label="Remove" className="h-7 w-7 inline-flex items-center justify-center rounded-lg text-text-tertiary hover:text-error-text hover:bg-white/60">
                  <Trash2 className="h-3.5 w-3.5" strokeWidth={2} aria-hidden />
                </button>
              </li>
            ))}
          </ul>
          <button type="button" onClick={() => patch("deliverables", [...data.deliverables, { id: `gd-${Date.now()}`, title: "", hours: 8 }])} className="mt-2 inline-flex items-center gap-1 h-7 px-2.5 rounded-lg border border-white/70 bg-white/55 backdrop-blur font-body text-[12px] font-semibold text-foreground hover:bg-white/75 transition-colors duration-fast">
        ...
        <section>
          ...
          <ul className="space-y-1.5">
            {data.clauses.map((c, i) => (
              <li key={c.id} className="rounded-xl border border-white/55 bg-white/55 backdrop-blur px-2.5 py-1.5 flex items-start gap-2 transition-colors duration-fast hover:bg-white/70">
...
function SuccessCard({
  ...
  return (
    <div className={cn(GLASS_CARD, "overflow-hidden p-6")} style={GLASS_SHADOW}>
...
function Footer({
  ...
  return (
    <footer className="flex items-center justify-between gap-3 px-5 sm:px-6 py-3.5 border-t border-white/55">
      <div className="flex items-center gap-2">
        {onBack && (
          <button type="button" onClick={onBack} className={ghostBtnClass}>Back</button>
        )}
        <button type="button" onClick={onCancel} className="inline-flex items-center justify-center h-10 px-3.5 rounded-2xl font-body text-[13.5px] font-semibold text-text-tertiary hover:text-foreground transition-colors duration-fast">Cancel</button>
      </div>
      <button
        type="button"
...
const cellCls = cn(
  "h-8 px-2 rounded-lg backdrop-blur-md",
...
function clauseClass(kind: ClauseKind): string {
  switch (kind) {
    ...
    case "constraint": return "border-white/70 text-text-secondary bg-white/55";
  }
}
