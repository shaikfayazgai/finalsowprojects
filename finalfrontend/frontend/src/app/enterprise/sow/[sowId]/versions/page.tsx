  ...
  if (error || !sow) {
    return (
      <div className="space-y-5 pb-12">
        <Link
          href="/enterprise/sow"
          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded font-body text-[12px] text-text-secondary hover:text-foreground hover:bg-white/50 transition-colors duration-fast"
        >
  ...
  if (!canView) {
    return (
      <div className="space-y-5 pb-12">
        ...
        <div className="rounded-2xl border border-warning-border bg-warning-subtle/40 backdrop-blur px-4 py-10 text-center">
          <p className="font-body text-[13px] font-semibold text-foreground">
    ...
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      <section className={cn(GLASS_CARD, "overflow-hidden")} style={GLASS_SHADOW}>
        <ul className="divide-y divide-white/60">
          {/* Active / current */}
          {activeDetail && (
            <li className="px-4 py-3 space-y-1.5 transition-colors duration-fast hover:bg-white/50">
              ...
              <div className="flex items-center gap-2 pt-1">
                <Link
                  href={`/enterprise/sow/${sow.id}`}
                  className={cn(
                    "inline-flex items-center gap-1.5 h-7 px-2.5 rounded-xl shrink-0",
                    "border border-white/70 bg-white/55 backdrop-blur",
          ...
          {Array.from({ length: Math.max(0, versionCount - 1) }).map((_, i) => {
            ...
            return (
              <li key={v} className="px-4 py-3 flex items-center gap-2 flex-wrap transition-colors duration-fast hover:bg-white/50">
