...
export function EnterpriseDashboardWorkspace() {
  ...
  return (
    <div className="space-y-5 pb-4 animate-fade-in">
      ...
      <div className={cn(DASH_CARD, "overflow-hidden")}>
        <div className="flex flex-col gap-1 px-4 sm:px-5 py-4 border-b border-stroke-subtle sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="font-body text-[13px] font-semibold text-foreground">Your queue</p>
      ...
      {shortcuts.length > 0 ? (
        <section aria-labelledby="ent-dash-shortcuts">
          <h2 id="ent-dash-shortcuts" className="mb-3 font-body text-[12px] font-medium uppercase tracking-[0.08em] text-text-tertiary">
            Shortcuts
          </h2>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {shortcuts.map((m) => (
              <ShortcutTile key={m.label} metric={m} />
            ))}
          </div>
        </section>
      ) : null}
...
function ShortcutTile({
  ...
  metric: { label: string; value: number; hint?: string; href?: string; tone: Tone; icon: LucideIcon };
}) {
  const Icon = metric.icon;
  const body = (
    <>
      <div className="flex items-center justify-between gap-2">
        <Icon className="h-4 w-4 text-text-tertiary group-hover:text-foreground transition-colors" strokeWidth={2} aria-hidden />
        <ArrowUpRight className="h-3.5 w-3.5 text-text-tertiary opacity-0 group-hover:opacity-100 transition-opacity" strokeWidth={2} aria-hidden />
      </div>
      <p className="mt-3 font-body text-[11px] font-medium uppercase tracking-wide text-text-tertiary">{metric.label}</p>
      <p className="mt-0.5 font-display text-[22px] font-semibold tabular-nums leading-none text-foreground">{metric.value}</p>
      {metric.hint ? (
        <p className="mt-1 font-body text-[11px]" style={{ color: metric.tone === "neutral" ? "var(--color-text-tertiary)" : TONE_TEXT[metric.tone] }}>
          {metric.hint}
        </p>
      ) : null}
    </>
  ...
  if (metric.href) {
    return (
      <Link href={metric.href} className={cn(DASH_CARD, "group block p-4 hover:bg-bg-subtle/40 transition-colors")}>
        {body}
      </Link>
    );
  }
  return <div className={cn(DASH_CARD, "p-4")}>{body}</div>;
}
