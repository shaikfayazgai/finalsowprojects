...
export function WorkforceCsvImport({ onApplied }: WorkforceCsvImportProps) {
  ...
  return (
    <section className={cn(GLASS_CARD, "overflow-hidden")} style={GLASS_SHADOW}>
      <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-5 py-4 border-b border-white/55">
        <div>
        ...
        <button
          ...
          className={cn(ghostBtnClass, "h-9 px-3 text-[12.5px]")}
        >
      ...
      <div className="p-5 space-y-3">
        {err && (
          <p
            role="alert"
            className="rounded-xl border px-3 py-2 font-body text-[12.5px] backdrop-blur"
        ...
        {errors.length > 0 && (
          <div
            className="rounded-xl border px-3 py-2 space-y-1 backdrop-blur"
          ...
          <div className="space-y-3">
            ...
            {total === 0 ? (
              ...
              <>
                ...
                {highDeletion && (
                  <p
                    className="rounded-xl border px-3 py-2 font-body text-[12px] text-text-secondary backdrop-blur"
                    style={{ background: TONE.warning.soft, borderColor: TONE.warning.border }}
                  >
                ...
                <div className="rounded-xl border border-white/55 bg-white/45 backdrop-blur overflow-hidden max-h-64 overflow-y-auto">
                  <ul className="divide-y divide-white/60">
                    {diffs.map((d) => (
                      <li
                        key={d.email}
                        className="px-3 py-2.5 flex flex-wrap items-center gap-2 gap-y-1 hover:bg-white/50 transition-colors"
                      >
                        ...
                        <span
                          className="inline-flex px-2 py-0.5 rounded-full border font-body text-[10px] font-semibold uppercase backdrop-blur"
                          style={{
                            background:
                              d.action === "create"
                                ? TONE.success.soft
