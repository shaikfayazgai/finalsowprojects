  ...
  return (
    <div className="space-y-5 pb-12 animate-fade-in">
      ...
      <section className={cn(GLASS_CARD, "overflow-hidden")} style={GLASS_SHADOW}>
        <div className="px-5 pt-3 pb-3 border-b border-white/55">
          <nav aria-label="Policy sections" className="flex flex-wrap gap-1">
            {VIEW_TABS.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "relative inline-flex items-center h-9 px-3.5 rounded-xl",
                    "font-body text-[13px] font-semibold whitespace-nowrap transition-colors duration-fast",
                    active ? "text-white" : "text-text-secondary hover:text-foreground hover:bg-white/55",
                  )}
    ...
    <div className={cn(GLASS_CARD, "overflow-hidden")} style={GLASS_SHADOW}>
      <div className="flex flex-col sm:flex-row sm:items-center divide-y sm:divide-y-0 sm:divide-x divide-white/60">
        <div className="flex items-start gap-3 px-5 py-4 min-w-0 flex-1">
          <span className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-white/55 bg-white/40 text-text-secondary shrink-0">
...
function SlaSection({
  ...
  return (
    <div className="px-5 py-4">
      ...
      <div
        aria-hidden
        className="hidden md:grid grid-cols-[minmax(0,1.4fr)_repeat(5,minmax(0,0.55fr))] gap-3 px-0 py-2 border-b border-white/55 mb-0"
      >
      ...
      <ul className="divide-y divide-white/60 border border-white/55 rounded-lg overflow-hidden">
        {templates.map((t) => (
...
function SlaRow({ template }: { template: SlaTemplateMock }) {
  ...
  return (
    <li className="md:grid md:grid-cols-[minmax(0,1.4fr)_repeat(5,minmax(0,0.55fr))] md:gap-3 md:items-center px-4 py-3 min-h-[48px] bg-white/40 hover:bg-white/50 transition-colors">
    ...
    <div className="px-5 py-4">
      ...
      <ul className="divide-y divide-white/60 border border-white/55 rounded-lg overflow-hidden">
        {rules.map((rule, index) => (
          <li key={rule.id} className="flex flex-wrap items-center gap-2 sm:gap-3 px-4 py-3 min-h-[48px] bg-white/40 hover:bg-white/50 transition-colors">
            <span className="inline-flex items-center justify-center min-w-[44px] h-7 px-2 rounded-full border border-white/55 bg-white/40 font-mono text-[10.5px] font-semibold text-text-secondary tabular-nums shrink-0">
              {rule.offset}
            </span>
